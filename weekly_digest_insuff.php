...
$subject = 'Weekly Insufficient checks list of all clients - from '.$fromDate.' to '.$toDate.'';
...
if($_REQUEST['send']=="yes"){
	...
	$where = "as_isdlt=0 AND v_isdlt=0 AND as_status='Insufficient' AND att_insuff=1 AND DATE(att.att_insuff_date) >= CURDATE() - INTERVAL DAYOFWEEK(CURDATE())+4 DAY AND DATE(att.att_insuff_date) < CURDATE() - INTERVAL DAYOFWEEK(CURDATE())-2 DAY GROUP BY vd.com_id";
	//echo "SELECT $cols FROM $tbls WHERE $where";
	$sel = $db->select($tbls,$cols,$where);
if(@mysql_num_rows($sel)>0){
$wholedata = '<table width="100%" border="0" >
					<tr>
					<td align="center" width="100%" colspan="20" style="border:none;">';
	...
	//	echo "<h2>Client #: ".$cc++."  Company Name: $com_name </h2><br>";
		...
		if(mysql_num_rows($user_info)>0){
       		...
       		$where2 = "vd.com_id='$com_id' AND as_isdlt=0 AND v_isdlt=0 AND as_status='Insufficient' AND att_insuff=1 AND DATE(att.att_insuff_date) >= CURDATE() - INTERVAL DAYOFWEEK(CURDATE())+4 DAY AND DATE(att.att_insuff_date) < CURDATE() - INTERVAL DAYOFWEEK(CURDATE())-2 DAY GROUP BY vc.as_id";
	...
	echo  $wholedata.'</td></tr></table>';
	  
	//emailTmp( $wholedata, $subject,$groupEmail,'','','','','Team');
	emailTmp( $wholedata, $subject,'khalique@xcluesiv.com','','','','','Khalique');
	//emailTmp( $wholedata, $subject,'hassan@xcluesiv.com','','','','','Hassan');
