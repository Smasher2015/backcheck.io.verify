<?php
	...
	define("BITRIX_URL",'http://my.backcheck.io/rest_api.php');
	die;
		$ch = curl_init();
...
function add_task($task_arr,$parent_id=0){
 ...
 $query_string="action=task_add&CREATED_BY=520&task_name=".$task_arr['task_name']."&desc=".$task_arr['task_desc']."&time_estimate=2&PARENT_ID=$parent_id&user_id=".$task_arr['user_id']."&tags=".$tag."&group_id=".$task_arr['group_id']."&START_DATE_PLAN=".date("Y-m-d")."&END_DATE_PLAN=".$enddateplan."&DEADLINE=$deadline&remainderdate=$remainderdate";
    curl_setopt($ch,CURLOPT_URL, BITRIX_URL);
    // Set a referer
   curl_setopt($ch, CURLOPT_HEADER, FALSE);
    // Should cURL return or print out the data? (true = return, false = print)
...
$row=mysql_query("select * from records where bitrixtid=0 limit 1");
while($check=mysql_fetch_array($row)){
	...
	if($check['component']=='education'){
					...
					$task_array['user_id']=522;
					$task_array['group_id']=12;
					$task_array['crawltype']=$check['crawltype'];
					//print_r($task_array);die;
					$bitrixctid=add_task($task_array,$leadid);
					mysql_query("update records set bitrixtid=$bitrixctid where primid=".$check['primid']."");
				//	$db->update("bitrixtid=$bitrixctid","records","primid=".$check['primid']."");		
	}
	...
	if($check['component']=='employment'){
					...
					$task_array['user_id']=490;
					$task_array['group_id']=13;
					$task_array['crawltype']=$check['crawltype'];
					//print_r($task_array);die;
					$bitrixctid=add_task($task_array,$leadid);
					mysql_query("update records set bitrixtid=$bitrixctid where primid=".$check['primid']."");
	...
	if($check['component']=='healthlicense'){
					...
					$task_array['crawltype']=$check['crawltype'];
					$task_array['user_id']=519;
					$task_array['group_id']=16;
					//print_r($task_array);die;
					$bitrixctid=add_task($task_array,$leadid);
					mysql_query("update records set bitrixtid=$bitrixctid where primid=".$check['primid']."");
				//	$db->update("bitrixtid=$bitrixctid","records","primid=".$check['primid']."");		
...
$crawl=mysql_query("select * from records where crawl_status='1' and bitrixtid!=0");
while($crawl_arr=mysql_fetch_array($crawl)){
	 ...
	 break;
		...
		$ch = curl_init();
 $query_string="action=updatetasktags&task_id=".$crawl_arr['bitrixtid']."&tags=".$tag."";
    curl_setopt($ch, CURLOPT_URL,BITRIX_URL);
    // Set a referer
   curl_setopt($ch, CURLOPT_HEADER, FALSE);
    // Should cURL return or print out the data? (true = return, false = print)
    ...
    curl_close($ch);
	mysql_query();
	mysql_query("update records set  crawl_status='0' where `bitrixtid`=".(int)$crawl_arr['bitrixtid']."");
}
?>