...
$table = '<table  border="1" style="border-collapse: collapse; border:1px solid #cccccc; border-color: #cccccc;">';
        ...
        while($analyst_arr = mysql_fetch_array($analyst)){
			$users2 = $db->select('users','user_id,concat(first_name,\' \',last_name) as fullname',"user_id='$analyst_arr[user_id]'");
			...
			$usersCount[$vals]['Not Init'][$fullname]	+= countChecks("vc.user_id='$rsUsers[user_id]' AND vc.v_id='$analyst_arr[v_id]'  AND  as_status = 'Open' AND as_vstatus='Not Initiated' AND as_isdlt=0");
		...
		//$db->update("delayed_reported=1","ver_checks","as_id=".$analyst_arr['as_id']);
			...
			if($Yesterday==$analyst_arr['as_pdate'] || $Yesterday==$analyst_arr['as_addate']){		
			$exclued_user_ids[] = $user_id;
			//echo $fullname."<br>".$analyst_arr['Submit Date'];
			$yesTerdayChecks[abc]['Not Init'][$fullname]	+= countChecks("vc.user_id='$rsUsers2[user_id]' AND vc.v_id='$analyst_arr[v_id]'  AND  as_status = 'Open' AND as_vstatus='Not Initiated' AND as_isdlt=0");
			$yesTerdayChecks[abc]['Insufficient'][$fullname]	+= countChecks("vc.user_id=$rsUsers2[user_id] AND vc.v_id='$analyst_arr[v_id]'  AND  as_status = 'Insufficient' AND as_vstatus='Not Initiated' AND as_isdlt=0");
			$yesTerdayChecks[abc]['WIP'][$fullname]	+= countChecks("vc.user_id='$rsUsers2[user_id]' AND vc.v_id='$analyst_arr[v_id]' AND as_status = 'Open' AND as_vstatus!='Not Initiated' AND as_isdlt=0 AND as_cldate IS NULL ");
			$yesTerdayChecks[abc]['Closed'][$fullname]	+= countChecks("vc.user_id='$rsUsers2[user_id]' AND vc.v_id='$analyst_arr[v_id]' AND as_status = 'Close' AND as_qastatus!='Rejected' AND as_isdlt=0 ");
...
$not_initiated .= '<td><table><tr><td colspan="3"><h3>'.$status.'</h3></td></tr>';
foreach($value as $fullname => $val){
$not_initiated .= '<tr><td><a href="'.SURL.'?action=advance&atype=search&vtrport=1&fullnam='.urlencode($fullname).'&status='.urlencode($status).'&frm=60&to=100000" >'.$val.'</a></td></tr>';
$totalarr[$fullname] += $val;
}
$not_initiated .='</table></td>';	
}
