<table style="width:100%" class="static table datatable-basic dataTable no-footer"><?php  echo "agaya";?></table><?php die;
...
}else if($where=="as_sent=4 AND as_cdnld=0 AND as_status='Close' AND as_qastatus = 'Approved'" || $where=="as_sent=4 AND as_cdnld=1 AND as_status='Close'" || $where=="as_sent=4 AND as_status='Close' AND (as_vstatus='negative' OR as_vstatus='match found' OR as_vstatus='record found')" || $where=="as_status = 'Close' AND as_sent=4 AND as_vstatus NOT IN ('negative', 'match found', 'record found')" || $riskwise==1){
...
<table style="width:100%" class="static table datatable-basic dataTable no-footer">
	<thead>
        <tr>
        <th>&nbsp;</th>
	...
	<?php 
        ...
        if(mysql_num_rows($checks)>0){
            while($checkInf = mysql_fetch_array($checks)){ 
				if($LEVEL==4){
					$onClick ="<a href='?action=details&case=$checkInf[v_id]&_pid=81'>".$checkInf['checks_title']."</a>";
				}else{
				...
				$days  = getDaysFromDates($today,$checkInf['as_addate'],$checkInf['com_id']);
				
				$closed_days  = getDaysFromDates($checkInf['as_cldate'],$checkInf['as_addate'],$checkInf['com_id']);
                ...
                <input type="hidden" name="rating" id="rating" value="'.$checkInf["as_rating"].'" />
                   ...
                   $prgresTD .= ' </ul>
					...
					$prgresTD = '<td > <a class="ctooltips" href="javascript:void(0);">					
						<button type="button" class="btn btn-success" onclick="'.$pdfClick.'"><i class="icon-cloud-download"></i> Single Report</button><span>Download Single Check Report</span></a><br /><br />
