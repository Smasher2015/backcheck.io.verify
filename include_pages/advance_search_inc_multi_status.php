...
if($IPAGE['m_where']!='') {
	...
	$check_status = 'all';
	function correctStatusLabel($st){
		...
		switch($st){
			...
			break;
			case 'Insufficient':
			$correctStatus = 'insufficient';
			break;
			case 'Not Init':
			$correctStatus = 'not_initiated';
			break;
			case 'WIP':
			$correctStatus = 'open';
			break;
		...
		//echo '$st:'.$st.' check_status:'.$_REQUEST['check_status'].' user_id:'.$_REQUEST['user_id'];
		
		return $correctStatus;
	}
if(isset($_REQUEST['vtrport'])){
	$_REQUEST['search_status']=1;
	if(isset($_REQUEST['dt'])){
	$_REQUEST['from_dt'] = urldecode($_REQUEST['dt']);	
	$_REQUEST['to_dt'] = urldecode($_REQUEST['dt']);
	}
	if(isset($_REQUEST['frm']) && isset($_REQUEST['to'])){
	$_days_frm = (int) $_REQUEST['frm'];	
	$_days_to = (int) $_REQUEST['to'];	
	
	$_frm = date('Y-m-d',strtotime(date('Y-m-d')."-$_days_frm days"));
	$_to = date('Y-m-d',strtotime($_frm." - $_days_to days"));
	$_REQUEST['from_dt'] = $_frm;
	$_REQUEST['to_dt'] = $_to;
	...
	$_REQUEST['check_status'] = correctStatusLabel(urldecode($_REQUEST['status']));
	}
	//SELECT user_id FROM users WHERE CONCAT(first_name,' ',last_name)='Saima Babar';

	if(isset($_REQUEST['fullnam'])){
	$sel = getInfo("users","CONCAT(first_name,' ',last_name)='".urldecode($_REQUEST['fullnam'])."'");
	$_REQUEST['user_id'] = array($sel['user_id']);	
	}
...
if(isset($_REQUEST['search_status']) || isset($_REQUEST['save_search_resuts'])){
	...
	$s_checks_id  	= (isset($_REQUEST['s_checks_id']))?$_REQUEST['s_checks_id']:array();
	$xclude_s_checks_id  	= (isset($_REQUEST['xclude_s_checks_id']))?$_REQUEST['xclude_s_checks_id']:array();
	
	$user_id  		= (isset($_REQUEST['user_id']))?$_REQUEST['user_id']:array();
	$client_id  	= (isset($_REQUEST['client_id']))?$_REQUEST['client_id']:array();
	...
	switch($value){
		...
		$pm_where .= " $Or (as_status = 'Close' AND as_qastatus!='Rejected' AND as_sent=4 AND as_isdlt=0) ";
		...
		case 'high_risk':
		$Or = ($pm_where!="")?' OR ':'';
		$pm_where .= " $Or (as_sent=4 AND as_status='Close' AND as_qastatus!='Rejected' AND as_isdlt=0 AND (as_vstatus='negative' OR as_vstatus='match found' OR as_vstatus='record found' OR as_vstatus='unsatisfactory' OR as_vstatus='positive match found')) ";
		$asDate = "as_cldate";
		break;
		case 'all':
		$Or = ($pm_where!="")?' OR ':'';
		
		$pm_where .= " $Or (as_isdlt=0)  ";
		$asDate = "as_addate";
		$asCLDate = "as_cldate";
		break;
		...
		case 'not_initiated':
		$Or = ($pm_where!="")?' OR ':'';
		$pm_where .= " $Or (as_status = 'Open' AND as_vstatus='Not Initiated' AND as_isdlt=0) ";
		$asDate = "as_addate";
		break;
		case 'insufficient':
		$Or = ($pm_where!="")?' OR ':'';
		$pm_where .= " $Or (as_status = 'Insufficient' AND as_vstatus='Not Initiated' AND as_isdlt=0)  ";
		$asDate = "as_addate";
		break;
		case 'open':
		$Or = ($pm_where!="")?' OR ':'';
		...
		case 'open_dash':
		$Or = ($pm_where!="")?' OR ':'';
		$pm_where .= " $Or (as_status != 'Close'  AND as_isdlt=0) ";
		$asDate = "as_addate";
		break;
		case 'close':
		case 'close_dash':
		$Or = ($pm_where!="")?' OR ':'';
		//$asDate = ($asDate!="as_cldate")?($check_status=='close_dash')?"as_addate":"as_cldate":'as_cldate';
		$asDate = ($asDate!="as_cldate")?($check_status=='close_dash')?"as_addate":"as_cldate":'as_cldate';
		$asCLDate = ($asDate!='as_cldate')?"as_cldate":"as_addate";
		$pm_where .= " $Or (as_status = 'Close' AND as_qastatus!='Rejected' AND as_isdlt=0)  ";
		//$asDate = ($check_status=='close_dash')?"as_addate":"as_cldate";
		$check_status = ($check_status=='close_dash')?"close":$check_status;
		break;
		case 'invoiced':
		$Or = ($pm_where!="")?' OR ':'';
		$asDate = ($asDate=="as_cldate")?'as_cldate':'as_addate';
		$pm_where .= " $Or (invoiced=1 AND as_status = 'Close' AND as_qastatus!='Rejected' AND as_sent=4 AND v_status='Close' AND as_isdlt=0)  ";
		//$asDate = "as_cldate";
		break;
	} // switch
	} // foreach
	...
	if($from_dt && $to_dt){
		if($asCLDate!='' && $asDate!='as_cldate'){
	$whDates = " AND (DATE_FORMAT($asDate, '%Y-%m-%d')".$whDates." OR DATE_FORMAT($asCLDate, '%Y-%m-%d')".$whDates.") ";	
		}else{
	...
	if(isset($_REQUEST['dt'])){
	$whDates = '';
	$pm_where2 = " AND (DATE(as_addate) = '".$from_dt."' OR DATE(as_pdate) = '".$from_dt."' OR DATE(as_cldate) = '".$from_dt."') ";		
	}else{
	$whDates = '';
	if($from_dt && $to_dt){
	$pm_where2 = " AND (DATE(as_addate) >= '".$to_dt."' AND DATE(as_addate) <= '".$from_dt."')";	
	}
	}
	}
	
	
	//DATE_FORMAT(as_addate, '%Y-%m-%d') BETWEEN '2016-04-26' AND '2016-04-26' 
	...
	if(!empty($xclude_s_checks_id)){
	$xclude_s_checks_ids = implode(",",$xclude_s_checks_id);	
	$xclude_s_checks_idWhere =	" AND  vc.checks_id NOT IN ($xclude_s_checks_ids)  ";
	...
	if(!empty(trim($pm_where))){
	$pm_where = ' ('.$pm_where.') '.$whDates.$clientWhere.$userWhere.$name_idWhere.$s_checks_idWhere.$xclude_s_checks_idWhere.$v_uadd_checks_idWhere;
	$pm_where2 = $pm_where2;
	//echo "4.pm_where: ".$pm_where4;
	}else{
	$pm_where = $pm_where3.$pm_where4.$whDates.$clientWhere.$userWhere.$name_idWhere.$s_checks_idWhere.$xclude_s_checks_idWhere.$v_uadd_checks_idWhere;
	//$pm_where2 = $pm_where2;	
	//echo "2.pm_where: ".$pm_where;
	}
	//echo "3.pm_where3: $pm_where3 <br> 4.pm_where: ".$pm_where4;	
}
if(is_array($check_status)){
	$check_status2 = implode("-",$check_status);
	$check_status = $check_status;
}else{
	$check_status = ($check_status=='open_dash')?'open':$check_status;
}
	//echo  'pm_where: '.$pm_where." pm_where2: ".$pm_where2;
	//echo $pm_where;
?>
...
$(document).ready(function() {
	...
	var table = $('#example').DataTable( {
		...
		"ajax": "actions.php?json_param=1&json_call=advance_search_case_info&action=fedit&fedit=s&pm_where=<?=base64_encode($pm_where)?>&pm_where2=<?=base64_encode($pm_where2)?>&m_orderby=<?=$m_orderby?>&_days=<?=$_days?>&check_status=<?=($check_status2!="")?$check_status2:$check_status?>",
		...
		"columns": [
			...
			"className":      'details-show'},
			{ "data": "reopened",
			"className":      'details-show',"orderable":      false,},
			{ "data": "closeddate",
			  "className":      'details-show'
			},
			{ "data": "modify_status",
    ...
    <div class="sidebar-detached" id="filters">
			...
			$checks = $db->select($tabls,"*",$where);	
			 ...
			 if(mysql_num_rows($checks)>0){
					...
					while($check = mysql_fetch_array($checks)){?>
						<option value="<?=$check['checks_id']?>" <?php echo (in_array($check['checks_id'],$s_checks_id))?'selected="selected"':'';?>><?=$check['checks_title']?></option>
					<?php }
			 }
											?>
            </select>
			 ...
			 <?php if($LEVEL!=4){?>
              ...
              <div class="form-group">
                  ...
                  <label>Select Client:</label>
                    <select id="source" name="client_id[]" multiple  class="select placeholder populate" onchange="getLocUsers(this.value);" placeholder="Select Client">
					  ...
					  <option value="<?php echo $client['id'];?>" <?php echo (in_array($client['id'],$client_id))?'selected="selected"':'';?>>
				...
				<div class="form-group">
				  ...
				  <label>Select Ops User:</label>
					...
					<?php
					  ...
					  <option value="<?php echo $user['user_id'];?>" <?php echo (in_array($user['user_id'],$user_id))?'selected="selected"':'';?>>
				...
				<?php */ } ?>
				 ...
				 <div class="form-group">
                   ...
                   <label>Select Status:</label>
                    <select id="check_status" name="check_status[]"  multiple  class="select placeholder populate">
                      <option value=""> Select Status </option>
					  <option value="all" <?php if(is_array($check_status)){ echo (in_array('all',$check_status))?'selected="selected"':'';} else {  echo ($check_status=='all')?'selected="selected"':''; }?>>
                     All
                      </option>
					   <option value="open" <?php if(is_array($check_status)){ echo (in_array('open',$check_status))?'selected="selected"':'';} else {  echo ($check_status=='open')?'selected="selected"':''; }?>>
                     Work in progress
                      </option>
					   <option value="not_initiated" <?php if(is_array($check_status)){ echo (in_array('not_initiated',$check_status))?'selected="selected"':'';} else {  echo ($check_status=='not_initiated')?'selected="selected"':''; }?>>
                     Not Initiated
                      </option>
					  <option value="insufficient" <?php if(is_array($check_status)){ echo (in_array('insufficient',$check_status))?'selected="selected"':'';} else {  echo ($check_status=='insufficient')?'selected="selected"':''; }?>>
                     Insufficient
                      </option>
					  <option value="close" <?php if(is_array($check_status)){ echo (in_array('close',$check_status))?'selected="selected"':'';} else {  echo ($check_status=='close')?'selected="selected"':''; }?> >
                     Close
                      </option>
					   <option value="invoiced" <?php if(is_array($check_status)){ echo (in_array('invoiced',$check_status))?'selected="selected"':'';} else {  echo ($check_status=='invoiced')?'selected="selected"':''; }?>>
                     Invoiced
                      </option>
                       <option value="no_risk" <?php if(is_array($check_status)){ echo (in_array('no_risk',$check_status))?'selected="selected"':'';} else {  echo ($check_status=='no_risk')?'selected="selected"':''; }?>>
                      No Risk
                      </option>
					    <option value="low_risk" <?php if(is_array($check_status)){ echo (in_array('low_risk',$check_status))?'selected="selected"':'';} else {  echo ($check_status=='low_risk')?'selected="selected"':''; }?>>
                      Potential Risk
                      </option>
                      <option value="high_risk" <?php if(is_array($check_status)){ echo (in_array('high_risk',$check_status))?'selected="selected"':'';} else {  echo ($check_status=='high_risk')?'selected="selected"':''; }?>>
                      High Risk
                      </option>
              ...
              <div class="form-group">	
				<div class="checkbox">
				<label for="send_weekly_update"><input type="checkbox" class="styled" name="send_weekly_update" id="send_weekly_update" value="1" <?=($_REQUEST['send_weekly_update']==1)?'checked="checked"':''?>>  Send me daily email of the saved search criteria. <span class="text-grey" href="#" title="You can check/uncheck this option from your profile section too." data-popup="tooltip" data-trigger="hover" data-container="body" data-placement="top"><i class="icon-info22"></i></span></label></div></div>
          ...
          <div class="content-detached">
                     ...
                     <div class="panel panel-flat">
						...
						<button type="submit" class="btn btn-default" onclick="javascript:;" name="downloadxcl" data-popup="tooltip" data-placement="top" data-trigger="hover" data-container="body" title="Download excel file"><i class="icon-file-excel position-left text-success"></i> Excel</button></span>
						<input type="hidden" name="pmwhere" value="<?=base64_encode($pm_where.$pm_where2)?>">
						...
						<input type="hidden" name="xcl_file_name" value="<?=($check_status2!="")?$check_status2:$check_status?>">
