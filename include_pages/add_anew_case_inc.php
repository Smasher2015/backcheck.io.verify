							 ...
							 <div class="col-lg-12 progress-bar-parent" >
								...
								if(mysql_num_rows($checks)>0){
									...
									$cc++;?>
										...
										<?php if(!in_array($company_id,$comids_allow_uncheck)){ echo 'onclick="this.checked=!this.checked;"'; } ?> checked="checked"   /> 
										
										<label for="<?=$num_check?>"><?=convertUCWords($check['checks_title'])?><label>  
...
<?php */?>                          
		  ...
		  <?php 
		  if(is_numeric($_REQUEST['case']) && $_REQUEST['case']!=0){  ?>
		  <input type="hidden"  name="is_duplicat" value="1"  />
		  <?php } ?>
