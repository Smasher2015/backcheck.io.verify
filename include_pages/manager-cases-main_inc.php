<?php
		...
		if(is_numeric($COMINF['id'])){
			$where="$IPAGE[m_where] com_id=$COMINF[id] $betw";
		}else{
			$where="1=1 $IPAGE[m_where] $betw";
		}
		
		if($_POST['status']!="") $where="$where AND as_vstatus='$_POST[status]'";
...
<div class="list-group">
        ...
        <div class="block">
            ...
            <div id="dt2" class="section">
                ...
                <form method="post" name="archiveCase"   enctype="multipart/form-data">    
                   ...
                   <table class="display datatable table table-bordered table-striped">
						...
						//echo "SELECT $cols FROM $tbls WHERE $where GROUP BY d.v_id ORDER BY d.v_id DESC"; exit;
                          //  $data = $db->select($tbls,$cols,"$where GROUP BY d.v_id ORDER BY d.v_id DESC");
							
							
							$db_count = $db->select($tbls,$cols,"$where GROUP BY d.v_id ORDER BY d.v_id DESC");
							$db_count =  mysql_num_rows($db_count);
							
							//$db_count = $db_count['cnt'];
							
							include("include_pages/pagination_inc.php");
												
							 $data = $db->select($tbls,$cols,"$where GROUP BY d.v_id ORDER BY d.v_id DESC $pages->limit");
			...
			<?php if($ACTION=='order' && $ATYPE=='certificate' && $db_count>0){ ?>
