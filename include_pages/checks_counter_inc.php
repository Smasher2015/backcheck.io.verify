<?php	
	...
	if($LEVEL==2){
		if(!isset($nAsgchk)) $nAsgchk = countChecks("ISNULL(user_id)",false);
	...
	if($LEVEL==4){	
		$closecas = countCases("((v_status='Close' AND v_sent=4) OR v_sent=2) AND v_cdnld=1");
		$redyecas = countCases("((v_status='Close' AND v_sent=4) OR v_sent=2) AND v_cdnld=0");
