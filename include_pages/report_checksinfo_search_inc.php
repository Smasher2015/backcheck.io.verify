...
?>
<div>
<table style="width:100%" class="static table  table datatable-basic tbl-inner">
	<thead>
        <tr class="danger">
        <th>&nbsp;</th>
<?php   $isSho = (($action=='assign' && $LEVEL==3) || ($LEVEL==2 && ($iware['m_action']=='assign' || $iware['m_action']=='assigned' || $iware['m_atype']=='send' || $iware['m_atype']=='ready' || $iware['m_action']=='problem' || $iware['m_action']=='search')));
...
<?php   }?>
    ...
    <tbody>
	<?php 
        $checks = searchCheckDetails($_REQUEST['case'],"","$where");
            ...
            while($checkInf = mysql_fetch_array($checks)){ 
				...
				<?php	}?>                    
                    ...
                    <td >
						...
						echo getColorClass($color);?>">
							<?=printableStatus($checkInf['as_status'],$checkInf['as_vstatus'])?>     
                        </span>
                    </td>
					<?php }?>
                ...
                <input type="hidden" name="rating" id="rating" value="'.$checkInf["as_rating"].'" />
                   ...
                   $prgresTD .= ' </ul>
					...
					$pdfClick = "downloadPDF('pdf.php?pg=case&ascase=$checkInf[as_id]')";
					$prgresTD = '<td ><ul class="icons-list text-center"><li><a class="ctooltips" href="javascript:void(0);" onclick="'.$pdfClick.' data-popup="tooltip" title="Download Single Check Report" data-placement="top"><i class="icon-file-download2"></i></a></li></ul>
                    </td>';
					echo $prgresTD;
					}
					?>
