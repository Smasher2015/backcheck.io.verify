...
<?php			
		...
		if($LEVEL==3 && $action!='assign') $where= (($where!='')?"$where AND ":"")."c.user_id=$_SESSION[user_id]";
        ...
        <?php 
            ...
            <tr>
                ...
                <td class="shover">
                    <?php if($csSts=='close'){ ?>
                        <div class="mainShrt">
                            <img style="cursor:pointer;width:35px;" onclick="<?=$pdfClick?>" title="Generate PDF" src="images/icons/<?=$PdfIcon?>">
							<?php 
...
<?php 	}		
		if(($action=='assign' && $LEVEL==3) || ($LEVEL==2  && ($action=='assign' || $action=='assigned' || $action=='problem'))){?>
