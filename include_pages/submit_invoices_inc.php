  ...
  <div class="row">
    <div class="col-md-12">
	  ...
	  <div class="page-section-title">
              <h2 class="box_head">Search Invoice #</h2>
            </div>
            ...
            <form action="?action=submitinvoices&atype=checks" class="table-form"  method="post">
              ...
              <div class="form-group">
                <div class="row">
                  ...
                  <div class="col-md-9">
                    <select id="as_id" name="as_id" class="form-control" >
                ...
                <div class="row">
                  ...
                  <div class="col-md-9">
                    <select id="as_status" name="as_status" class="form-control" >
                          ...
                          <tbody>
                            ...
                            <tr>
							 ...
							 $selCot = $db->select('clients_checks','clt_cost'," com_id=".$re['com_id']." AND checks_id='".$re['checks_id']."'");
							  ...
							  <input type="text" style=" border: 1px solid #dddddd; text-align:center;" onkeyup="this.value=this.value.replace(/[^\d]/,'');" size="5" id="cost_id_<?=$re['as_id']?>" name="cost[<?=$re['as_id']?>]" value="<?=($rsCost['clt_cost']!="")?$rsCost['clt_cost']:0?>" readonly>
							  </td>
							  <td><?php 
							 ...
							 $selCot = $db->select('clients_checks','clt_cost'," com_id=".$re['com_id']." AND checks_id='".$re['checks_id']."'");
							  ...
							  $rsUniFee = mysql_fetch_assoc($selUniCost);
							   ...
							   //echo $cost;
							  ?>
							  <input  type="text" style=" border: none;" onkeyup="this.value=this.value.replace(/[^\d]/,'');" size="5" id="cost_id_<?=$re['as_id']?>" name="cost[<?=$re['as_id']?>]" value="<?=$cost?>" readonly="readonly">
							  </td>
    ...
    <div class="modal-content">
      <form name="" method="post" id="sub_inv" >
        <div class="modal-header">
          ...
          <h4 class="modal-title"><i class="icon-plus"></i><span>Merge Checks</span></h4>
        </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <form name="" method="post" id="sub_inv" >
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
          <h4 class="modal-title"><i class="icon-plus"></i><span>Submit Invoice Number</span></h4>
        </div>
        <div class="modal-body">
          <div id="msg-inv"></div>
        <?php /*   <div class="form-group">
...
function calculateAmount(isTax){
	...
	// taxAmount = parseFloat(Tax)* totalAmount / netTax;
	 
	 subTotal = totalAmount+taxAmount;
	 $('#loadAmount').html(' <i class="icon-money"></i><h2>TOTAL AMOUNT<br>WITH TAX</h2><div class="counter counter-small">Rs. '+subTotal.toFixed(2)+'</div>');
	 
	 }else{
		
	subTotal = totalAmount;		
	 $('#loadAmount').html(' <i class="icon-money"></i><h2>TOTAL AMOUNT<br>WITHOUT TAX</h2><div class="counter counter-small">Rs. '+subTotal.toFixed(2)+'</div>');	 
	 }
