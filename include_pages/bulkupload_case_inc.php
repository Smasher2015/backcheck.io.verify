<?php 
if(!isset($_REQUEST['upload_bulk'])){
	echo "<script> document.location='".SITE_URL."?action=add&atype=easy_upload&errr=1'; </script>";
	exit;
}
if($LEVEL!=4){
      ...
      <div class="panel panel-white">
		...
		<div class="clearFix"></div>
			<div class="col-md-12" style="position: absolute;right: 0;width: auto;top: 0;"><fieldset class="text-right">
                  <button type="submit" class="btn bg-success-600 btn-lg check_cnic" name="bulkupload_case_only" ><i class="icon-add-to-list position-left"></i> Submit Checks</button>
                ...
                <div class="section-sprate">
									<div class="col-md-12">
											...
											<?php /*?><?=$cc?><?php */?> <h6><label class="checkbox-inline"><input type="checkbox" class="styled clss_hide_remov" name="skip_case[<?=($lCount-1)?>]" value="<?=($lCount-1)?>">Skip this case</label></h6>
                                                ...
                                                <input type="text" name="ename<?=($lCount-1)?>" placeholder="Applicant Name" value="<?=$csv_line[0]?>" id="ename<?=$c?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?>" required>
												...
												<input type="text" name="fname<?=($lCount-1)?>" placeholder="Father Name"  value="<?=$csv_line[1]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?>" required></div></div>
												<div class="col-md-4">
                                                <div class="form-group">
												<label><?php echo ID_CARD_NUM;?>:</label>
												<input type="text" name="cnic<?=($lCount-1)?>" placeholder="<?php echo ID_CARD_NUM;?>"  value="<?=$csv_line[2]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?>" maxlength="50" title="<?php echo ID_CARD_NUM;?>">
												...
												<input type="text" name="empcode<?=($lCount-1)?>" placeholder="<?php echo CLIENT_REF_NUM;?>"   value="<?=$csv_line[3]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?>" required maxlength="20"></div></div>
												
												<div class="col-md-4">
                                                <div class="form-group">
												<label>Comments.</label>
												<textarea name="comments<?=($lCount-1)?>" class="form-control" placeholder="Comments" title="Type your comments" ><?=$csv_line[4]?></textarea></div></div>
...
<?php include("include_pages/bulkupload_scripts2.php")?>
