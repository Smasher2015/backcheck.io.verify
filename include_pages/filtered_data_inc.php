<?php
$company_id = (isset($_REQUEST['com_id']))?$_REQUEST['com_id']:'';
...
switch($_REQUEST['filter_what']){
	...
	case"ready_download";
			$selFilters = getFiltersBy('ready_download','as_cldate',$company_id);
			$addFilter = ($selFilters['filter_where'])?$selFilters['filter_where']:''; 
			
			$data = client_checks_info("as_sent=4 AND as_cdnld=0 AND as_status='Close' $addFilter $comWhere","LIMIT 4");
			if($data){
	...
	case"progress_overview":
			$selFilters = getFiltersBy('progress_overview','as_cldate',$company_id);
			$selAll = getFiltersBy('progress_overview','as_addate',$company_id);
			$addFilter = ($selFilters['filter_where'])?$selFilters['filter_where']:''; 
			$addFilter_all = str_replace('','',$selAll['filter_where']);
						...
						$submitted_checks 		= countChecks(" 1=1 ".$addFilter_all."  $comWhere");
						$completed_checks 		= countChecks("as_status='Close' $addFilter $comWhere");
						...
						$closecas = countCases("(v_status='Close' AND v_sent=4 $addFilter) $comWhere");
						//For Progress
						$user_id = $_SESSION['user_id'];
						$user_info = getUserInfo($user_id);
						$user_com_id = $user_info['com_id'];
						$total = countCases("as_isdlt = 0 $addFilter $comWhere");
		...
		case"work_in_progress":	
			...
			$selFilters = getFiltersBy('work_in_progress','as_pdate',$company_id);
			$addFilter = ($selFilters['filter_where'])?$selFilters['filter_where']:''; 
								//echo $addFilter;
								$data = client_checks_info("as_status!='close' AND as_sent!=4 AND v_isdlt=0  $addFilter $comWhere","LIMIT 4");
								if($data){
									  while($row = mysql_fetch_assoc($data)){ 
									  ?> 
		...
		case"by_risk":
				...
				$cols ="$cols WHERE  $where AND as_isdlt=0  $addFilter  GROUP BY mnth,sts ORDER BY v_date DESC LIMIT 36) DATA"; */
	...
	$discrepancy_checks 	= countChecks("as_vstatus='Discrepancy' $addFilter $comWhere");
	$need_attention_checks 	= countChecks("as_status='problem' $addFilter $comWhere");
	//echo  " 1=1 $addFilter_all  $comWhere";
	$submitted_checks 		= countChecks(' 1=1 '.$addFilter_all."  $comWhere");
