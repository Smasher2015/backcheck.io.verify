                 ...
                 <div>
                     <div class="list-group-item">
                      ...
                      <table class="table table-bordered table-striped" id="tableSortable">
                        ...
                        <tbody>
                          <?php	
						$checks = $db->select("savvion_check","*","team_lead_id = $_SESSION[user_id]");
                        ...
                        while($check = mysql_fetch_array($checks)){ ?>
                          <tr>
							...
							<td <?php if($check['user_id']!=0) echo "style='color:#92c83e;'";  else  echo  "style='color:#ff0000;'";?>><?php
							if($check['user_id']){
							$uInfo  = getUserInfo($check['user_id']);
							$user	= ucwords($uInfo['first_name']);
							echo $user;
							}else echo "Not Delegate";?></td>
                          ...
                          <?php } ?>
                      		...
                      		<div class="form-group">
                                    ...
                                    <?php $analyst = $db->select("users u inner join tl_savvion_analyst_relation sv on u.user_id=sv.analyst_id ","*"," is_active = 1");
											if(mysql_num_rows($analyst)>0){
											while($tlead = mysql_fetch_array($analyst)){ 
											?>
                                            <option value="<?=$tlead['user_id']?>"><?=ucwords($tlead['first_name'].' '.$tlead['last_name'])?></option>
										<?php 	}
										}
									?>
                     		 ...
                     		 <button type="submit" class=" btn btn-success dropdown-toggle"  name="delegateSavvionChecks" > <span>Delegate checks to analyst</span> </button>
							 </footer>
                      </form>
                    </div>
        		</div>
