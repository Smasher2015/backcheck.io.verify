        ...
        <input type="hidden" name="missystem" value="yes" />
          ...
          <table class="table table-bordered table-striped" id="tableSortable">
			...
			//if(0>1){
               ...
               <tr>
			<td><input type="checkbox" name="record_id[]" id="record_id_<?=$cc?>" class="cheks" value="<?=$re['misID']?>" />  </td>
					<td><?=$inx?></td>
...
<?php */?>                    </td>
                   ...
                   <?php */?> </td>
                    <td>
                    <select name="status_<?=$re['misID']?>" id="status_<?=$re['misID']?>" onblur="mainfunction('edit','<?=$re['misID']?>')" class="select">
                    	<option value="-">-- Select Status --</option>
<!--                    	<option value="Not Initiated">Not Initiated</option>
                    	<option value="WIP">WIP</option>
                    	<option value="Insufficient">Insufficient</option>
-->           		  
					...
					<td>
                    <?php /*?><select name="ianame_<?=$re['misID']?>" id="ianame_<?=$re['misID']?>" class="select">
                    	<option value="-">-- Select IA Name --</option>
					...
					<input type="hidden" name="ianame[]" id="ianame_<?=$re['misID']?>"  value="<?=urldecode($re['ianame'])?>" class="each_ianame" />
...
<?=$re['qualification_detail']?>
					...
					<td><?php /*?><input name="fathername_<?=$re['misID']?>" id="fathername_<?=$re['misID']?>" placeholder="Father Name" value="<?=$re['fatherName']?>"/><?php */?><?=urldecode($re['fatherName'])?></td>
					...
					<td><input type="text" name="ddamount_<?=$re['misID']?>" id="ddamount_<?=$re['misID']?>" placeholder="DD Amount" value="<?=$re['ddamount']?>"/><input type="hidden" name="dd_amount[]"  value="<?=$re['ddamount']?>"/></td>
                    
					<td><input type="text" name="ddnumber_<?=$re['misID']?>" id="ddnumber_<?=$re['misID']?>" placeholder="DD Number" value="<?=$re['ddnum']?>"/></td>
                    
					<td><input type="text" name="letter_ref_num_<?=$re['misID']?>" id="letter_ref_num_<?=$re['misID']?>" placeholder="Letter Ref. Number" value="<?=$re['letter_ref_num']?>" /><input type="hidden" name="letter_ref[]"   value="<?=$re['letter_ref_num']?>" /></td>
...
function download_reference_letter(){
	...
	/* $('.each_ianame').each(function(ind,obj) {
		console.log($('#record_id_'+ind));
		if ($('#record_id_'+ind).is(":checked")){
		each_ianame.push($(obj).val());
		};                        
    }); 
