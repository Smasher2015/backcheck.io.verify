		...
		$len = strlen($_FILES["bulk_file"]["name"]);
			...
			<div class="form-group mt-20">
                ...
                <button type="button" class="btn bg-red" onclick="document.location='files/format/applicant-bulkupload-sample.csv'"><i class="icon-file-download position-left"></i> Download Sample Format</button></div>
							...
							<h4 class="border-bottom border-grey" style="padding-bottom:7px;margin-bottom:18px; display:none;">Select Checks</h4>
							...
							$where = "cc.com_id=$company_id AND ck.is_active=1 AND cc.clt_active=1";
								...
								$cc=0;
									...
									'checks_title' => $check['checks_title'],
									'checks_info' => $check['checks_info'],
								...
								<div class="checkbox bulk_check col-md-4" style="display:none;">No Checks in package.</div>
								<?php } ?>
			...
			$values='';
                ...
                <div class="section-sprate">
								...
								<div class="checkbox col-md-4">
											<label>
												<input type="checkbox" checked class="styled parsley-validated parsley-error"  value="<?=$check['checks_id']?>" name="checks<?=($lCount-1)?>[]" data-parsley-required="true" data-parsley-error-message="You must select at least one check" onchange="disbaleCheckFields(this,<?=($lCount-1)?>,<?=$check['checks_id']?>);">
												<?=convertUCWords($check['checks_title'])?>   <?php if(!empty($check['checks_info'])){ ?>
                                            	<a class="ctooltips" href="#"><i class="icon-info22"></i><span><?=$check['checks_info']?> </br> 
												Amount: <?php echo $chk_price;?>
												</span></a>
                                            <?php } ?>
								...
								<div class="checkbox  col-md-4">No Checks in package.</div>
								<?php }  ?>
								</div>
								<?php 
										
										if(in_array(1,$allchecks)){?>
                                        <div class="col-md-6">
												...
												<input type="text" name="uni_name<?=($lCount-1)?>" placeholder="University Name"  value="<?=$csv_line[7]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?> dis_not_<?=($lCount-1)?>_1" >
                                                </div></div>
												<div class="col-md-6">
                                                <div class="form-group">
												<label>Registration Number:</label>
												<input type="text" name="reg_num<?=($lCount-1)?>" placeholder="Registration Number"  value="<?=$csv_line[8]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?> dis_not_<?=($lCount-1)?>_1" >
                                                </div></div>
												...
												<input type="text" name="degree<?=($lCount-1)?>" placeholder="Degree Title"  value="<?=$csv_line[9]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?> dis_not_<?=($lCount-1)?>_1"  title="Type Qualification e.g: BSC,MSC,BCS">
                                                </div></div>
												<div class="col-md-6">
                                                <div class="form-group">
												<label>Remarks:</label>
												<input type="text" name="remarks<?=($lCount-1)?>" placeholder="Remarks"  value="<?=$csv_line[10]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?> dis_not_<?=($lCount-1)?>_1"  title="Type Remarks e.g: Passed,Fail">
												...
												<input type="text" name="pass_year<?=($lCount-1)?>" placeholder="Passing Year"  value="<?=$csv_line[11]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?> dis_not_<?=($lCount-1)?>_1"  title="">
                                                </div></div>
												<div class="col-md-6">
                                                <div class="form-group">
												<label>Serial # / Seat # / Roll #</label>
												<input type="text" name="serial_no<?=($lCount-1)?>" placeholder="Serial No"  value="<?=$csv_line[12]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?> dis_not_<?=($lCount-1)?>_1"  title="">
                                                </div></div>
											...
											<?php } if(in_array(2,$allchecks)){?>
												...
												<input type="text" name="company_name<?=($lCount-1)?>" placeholder="Company Name"  value="<?=$csv_line[13]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?> dis_not_<?=($lCount-1)?>_2" >
                                                </div></div>
                                                
												<div class="col-md-6">
                                                <div class="form-group">
												<label>Date of Joining:</label>
												<input type="text" name="date_of_join<?=($lCount-1)?>" id="date_of_join<?=$c?>" placeholder="Date of Joining (YYYY-MM-DD)"  value="<?=$csv_line[14]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?> date datetimepicker-month dis_not_<?=($lCount-1)?>_2"   title="Type Date of Joining (YYYY-MM-DD)">
                                                </div></div>
												<!--]-->
												...
												<input type="text" name="emp_status<?=($lCount-1)?>" placeholder="Employement Status"  value="<?=$csv_line[15]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?> dis_not_<?=($lCount-1)?>_2" >
                                                </div></div>
												<div class="col-md-6">
                                                <div class="form-group">
												<label>Last Working Day:</label>
												<input type="text" name="last_work_day<?=($lCount-1)?>" placeholder="Last Working Day"  value="<?=$csv_line[16]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?> dis_not_<?=($lCount-1)?>_2" >
                                                </div></div>
												
												<div class="col-md-6">
                                                <div class="form-group">
												<label>Last Designation:</label>
												<input type="text" name="last_designation<?=($lCount-1)?>" placeholder="Last Designation"  value="<?=$csv_line[17]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?> dis_not_<?=($lCount-1)?>_2" >
                                                </div></div>
												<div class="col-md-6">
                                                <div class="form-group">
												<label>Last Place of Posting:</label>
												<input type="text" name="last_place_posted<?=($lCount-1)?>" placeholder="Last Place of Posting"  value="<?=$csv_line[18]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?> dis_not_<?=($lCount-1)?>_2" >
                                                </div></div>
												<div class="col-md-6">
                                                <div class="form-group">
												<label>Last Drawn Salary:</label>
												<input type="text" name="last_drawn_salary<?=($lCount-1)?>" placeholder="Last Drawn Salary"  value="<?=$csv_line[19]?>" class="blnk form-control clss_hide_remov<?=($lCount-1)?> dis_not_<?=($lCount-1)?>_2" >
                                                </div></div>
...
<script>
function disbaleCheckFields(ths,cnt,chk){
	//console.log($(ths));
	//var thisCheck = ths;
		//	var vals = thisCheck.value;
		if(chk==1 || chk==2){
			
	  if ( $(ths).is(':checked') ) {
		 //console.log('.dis_not_'+cnt+'_'+chk);
		$('.dis_not_'+cnt+'_'+chk).removeAttr('disabled');
		  
	  }
	  if ( ! ($(ths).is(':checked')) ) {
		 //console.log('.dis_not_'+cnt+'_'+chk);
		$('.dis_not_'+cnt+'_'+chk).attr('disabled','disabled');
