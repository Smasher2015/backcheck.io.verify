<?php 
	...
	if($_REQUEST['qa_return_status']!=0){
	$qa_return_status_col 		= 'qa_return_status,'; 
	$qa_return_status_val 		= "'".$_REQUEST['qa_return_status']."',"; 
	
	$qa_return_count_col 		= 'qa_return_count,'; 
	$qa_return_count_val 		= "'1',";
	if(isset($_REQUEST['savvion_check_id'])){
	$rsc = mysql_fetch_assoc($db->select("savvion_check","qa_return_counter","id=$savvion_check_id"));
	$counterVal = (int) $rsc['qa_return_counter']+1;
	$qa_return_counter_col 		= 'qa_return_counter,'; 
	$qa_return_counter_val 		= "'$counterVal',";
	
	}
	
	
	}else{
	$qa_return_status_col 		= ""; 
	$qa_return_status_val 		= ""; 	
	$qa_return_count_col 		= ""; 
	$qa_return_count_val 		= "";
	$qa_return_counter_col 		= ""; 
	$qa_return_counter_val 		= "";
	}
	}
	...
	$Barcode 				= urlencode(utf8_encode($_REQUEST['Barcode'])); 
	$ApplicantName 			= $_REQUEST['ApplicantName'];
	$SubBarcode 			= urlencode(utf8_encode($_REQUEST['SubBarcode'])); 
	$ClientName 			= urlencode(utf8_encode($_REQUEST['ClientName'])); 
	$ClientRefNo 			= urlencode(utf8_encode($_REQUEST['ClientRefNo'])); 
	$DateOfBirth 			= $_REQUEST['DateOfBirth']; 
	$PassportNo 			= urlencode(utf8_encode($_REQUEST['PassportNo'])); 
	$AKAName 				= $_REQUEST['AKAName']; 
	$Gender 				= $_REQUEST['Gender']; 
	$Nationality 			= urlencode($_REQUEST['Nationality']); 
	$ArabicName 			= urlencode(utf8_encode($_REQUEST['ArabicName'])); 
	$lastupdate 			= urlencode(utf8_encode($_REQUEST['lastupdate'])); 
	$assigndate 			= urlencode(utf8_encode($_REQUEST['assigndate'])); 
	$duedate 				= urlencode(utf8_encode($_REQUEST['duedate'])); 
	$_UNBOUND_textArea3 	= urlencode(utf8_encode($_REQUEST['_UNBOUND_textArea3'])); 
	$HistoryRemarks 		= urlencode(utf8_encode($_REQUEST['HistoryRemarks'])); 
	$_UNBOUND_historyNew 	= urlencode(utf8_encode($_REQUEST['_UNBOUND_historyNew'])); 
	$CommentsForECT 		= urlencode(utf8_encode($_REQUEST['CommentsForECT'])); 
	$is_error 				= urlencode(utf8_encode($_REQUEST['is_error'])); 
	$is_duplicate 			= urlencode(utf8_encode($_REQUEST['is_duplicate'])); 
	//$extra_qc_comments 			= urlencode(utf8_encode($_REQUEST['extra_qc_comments'])); 
	$is_this_duplicate 			= $_REQUEST['is_this_duplicate']; 
	...
	//$array_combine 					= array_map(function($key, $val) {return array($key=>$val);}, $savvion_key, $savvion_value);
	//$array_combine 					= array_combine($savvion_key,$savvion_value);
	$array_combine = array();
	foreach($savvion_key as $key => $val){
	$array_combine[$key][$val] = $savvion_value[$key];	
	...
	foreach($array_combine as $key=>$vl){
		
		//var_dump( $vl[$savvion_key[$key]]);
		//echo "<br> savvion_key: ";
		//var_dump( $savvion_key[$key]);
		
		//echo "<br><br> savvion_value: ";
		//var_dump( $savvion_value[$key]);
		
	}
	
	
	
	//echo 'array_combine', print_r($array_combine); die;
	
	if($savvion_check_id){
		$savvion_editkey 					= $_REQUEST['editkey'];
		
		$edit_array_combine = array();
	foreach($savvion_key as $key => $val){
	$array_combine[$key][$val] = $savvion_value[$key];	
	...
	/* Attachment */ 
		$ids 							= $_REQUEST['ids'];
		$attach_comments 				= $_REQUEST['attach_comments'];
		...
		$result_att = array();
			foreach ($ids as $id => $key) {
				...
				'attach_comments'    => 	$_REQUEST['attach_comments'][$id],
	...
	$verbal_status 					= urlencode($_REQUEST['verbal_status']);
	...
	$Phraseology = ($Phras['phar']!="")?$Phras['phar']:0;
	//var_dump($Phraseology); exit;
	...
	$ModeOfCommunication 			= urlencode(utf8_encode($_REQUEST['ModeOfCommunication']));
	$MandatoryRequirements			= urlencode(utf8_encode($_REQUEST['MandatoryRequirements']));	
	$ClientDiscrepancy				= urlencode(utf8_encode($_REQUEST['ClientDiscrepancy']));
	$SpecialInstructions			= urlencode(utf8_encode($_REQUEST['SpecialInstructions']));
	$QC2Comment						= urlencode(utf8_encode($_REQUEST['QC2Comment']));
	$QC3Comment						= urlencode(utf8_encode($_REQUEST['QC3Comment']));
	...
	$modeNumber 					= urlencode($_REQUEST['modeNumber']);
	...
	$VerificationComment 			= urlencode(utf8_encode($_REQUEST['VerificationComment']));
	$inDivPhraseology 				= urlencode(utf8_encode($_REQUEST['inDivPhraseology']));
	$InsufficiencyComment 			= urlencode(utf8_encode($_REQUEST['InsufficiencyComment']));
		...
		//echo $values; exit;
			...
			if($_REQUEST['ERR']==''){
					...
					if(mysql_num_rows($check_data)>0 && $savvion_check_id==''){
						...
						if($VStatus!='' && $VStatus!='Work_in_Progress'){
						$arrr = array('Inaccessible_for_verification','Not_Initiated','Pending_for_Authorization','Pending_for_Reply');
						if($Phraseology==0 || $Phraseology=="" ){
							
						if(!in_array($VStatus,$arrr)){
						msg('err',"Please select Phraseology under verification status!");	
						}
						}
						if($VerificationLanguage==0 || $VerificationLanguage==""){
						msg('err',"Please select Verification Language under verification status!");	
						}
						if($ModeOfVerification==0 || $ModeOfVerification==""){
						msg('err',"Please select Mode Of Verification under verification status!");	
						}
						if($initiatedByName==0 || $initiatedByName==""){
						msg('err',"Please select Initiated By Name under verification status!");	
						}
					}else{
				// edit savvion check
				if($savvion_check_id){
					...
					// -------------- edit and sent to qc ---------------------//
						if($sentaddsavvioncheck){
							...
							foreach($result_att as $attach){
								...
								$attach_comments = 	$attach['attach_comments'];
								$cols_attach = "
									sv_id,
									at_attachment,
									attach_comments
							...
							// -------------------- edit savvion check only ------------------------//
							
							$cols .= ',status,bot_status';
							$values .= ',2,1';
							
							//echo "cols: ". $cols."<br> vals: ".$values; exit;
							//echo "update savvion_check set ($cols) values($values) where id=$savvion_check_id"; exit;
							$isAddEdit = $db->updateCol($cols,$values,'savvion_check',"id=$savvion_check_id");
							foreach($result as $data){
								$key = $data['savvion_key'];
								$val = $data['savvion_value'];
								...
								$cols_ver = "savvion_key,savvion_value";
								$value_ver = "'$key','$val'";
								//echo "UPDATE savvion_check_values set ($cols_ver) value ($value_ver) WHERE savvion_check_id=$savvion_check_id and scv_id=$savvion_editkey <br>";
								$isEditKey = $db->updateCol($cols_ver,$value_ver,'savvion_check_values',"savvion_check_id=$savvion_check_id and scv_id=$savvion_editkey");
							}
							//exit;
							foreach($result_att as $attach){
						...
						// -------------------- insert and sent to qc savvion check ------------------------//
						if($sentaddsavvioncheck){
							...
							$value_ver = "$in_id,'$savvion_key[$key]','$savvion_value[$key]'";
	...
	function getcheckname($checkID,$savv_check_id){
		...
		if(is_numeric($savv_check_id)){
				...
				$editValues   = $db->select("savvion_check_values","*","savvion_check_id=$savv_check_id order by scv_id asc");
		...
		while($Qua =mysql_fetch_array($Qualification)){ 
			...
			if($Qua['sq_id']==26 || strtolower($Qua['sq_name'])=='other' || strtolower($Qua['sq_name'])=='others'){
				$qua_options .= 'selected="selected"';
		...
		$education = '<div  id="eduction">
                                ...
                                <h4 class="section-title">Education</h4>
                                    ...
                                    <div class="form-group">
                                    	...
                                    	<textarea name="IA_Name" class="form-control parsley-validated" rows="4" data-parsley-minwords="4" data-parsley-required="true" placeholder="University Name">'.$data['IA_Name'].'</textarea>
                                    </div>
                                    <div class="form-group">
									...
									<div class="savvion-eduction-box3-1">
										...
										<div class="form-group">
											...
											<div class="btn_move"><i class="icon-share-alt"></i></div>
											<select id="color" class="form-control quali" name="'.$save_val_sel.'" onchange="hide_div(this.value,\'quali\')">
												'.$qua_options.'
											</select>
										...
										<div class="form-group">
											...
											<div class="btn_move"><i class="icon-share-alt"></i></div>
											<select class="form-control deg_options"  name="savvion_value[]">
												'.$deg_options.'
											</select>
										...
										<div class="form-group">
											...
											<input type="text" name="savvion_key[]" id="Conferred-Date" value="'.$savvion_key_edit[4].'"  class="datetimepicker-month form-control" placeholder="Conferred Date">
											<div class="btn_move"><i class="icon-share-alt"></i></div>
											<input type="text" name="savvion_value[]"  value="'.$savvion_value_edit[4].'"  class="datetimepicker-month form-control" placeholder="Conferred Date Value">
											<input type="hidden" name="editkey[]" value="'.$savvion_value_id[4].'" />
										</div>
										<div class="form-group">
											<label for="Attendance-From">Attendance From:  </label>
											<input type="text" name="savvion_key[]" id="Attendance-From" value="'.$savvion_key_edit[5].'"  class="datetimepicker-month form-control" placeholder="Attendance From">
											<div class="btn_move"><i class="icon-share-alt"></i></div>
											<input type="text" name="savvion_value[]" value="'.$savvion_value_edit[5].'"  class="datetimepicker-month form-control" placeholder="Attendance From Value">
											<input type="hidden" name="editkey[]" value="'.$savvion_value_id[5].'" />
										</div>
										<div class="form-group">
											<label for="Attendance-To">Attendance To:  </label>
											<input type="text" name="savvion_key[]" id="Attendance-To" value="'.$savvion_key_edit[6].'"  class="datetimepicker-month form-control" placeholder="Attendance To">
											<div class="btn_move"><i class="icon-share-alt"></i></div>
											<input type="text" name="savvion_value[]" value="'.$savvion_value_edit[6].'"  class="datetimepicker-month form-control" placeholder="Attendance To Value">
		...
		while($Desig =mysql_fetch_array($Designation)){ 
			...
			$Desig_options .= '<option  value="'.$Desig['sd_id'].'" ';
			 
			 
			 if(is_numeric($savvion_value_edit[0])){
				
			if($savvion_value_edit[0] == $Desig['sd_id']){
				$Desig_options .= 'selected="selected"';
				$showHid_Div = 'display:none;';
				$otherVal = "";
				$save_val_sel="savvion_value[]";
				$save_val_txt="";
			...
			if($Desig['sd_id']==17 || strtolower($Desig['sd_designation'])=='other' || strtolower($Desig['sd_designation'])=='others'){
				$Desig_options .= 'selected="selected"';
				$showHid_Div ='display:block;';
				$otherVal = $savvion_value_edit[0];
				$save_val_sel="";
				$save_val_txt="savvion_value[]";
			}
			}
			 
			 $Desig_options .= ' >'.ucwords($Desig['sd_designation']).'</option>';  
								...
								<div class="savvion-employment-box2-1">
									...
									<div class="savvion-employment-box3-1">
										...
										<div class="form-group">
											...
											<input type="text" name="savvion_key[]" id="Designation" value="'.$savvion_key_edit[0].'"  class="datetimepicker-month form-control" placeholder="Designation" />
											<input type="hidden"  value="'.$savvion_value_id[0].'" />
											
											<div class="btn_move"><i class="icon-share-alt"></i></div>
											<select class="form-control desig" name="savvion_value[]" onchange="hide_div(this.value,\'designation\')">
												'.$Desig_options.'
											</select>
											<div id="temp_div" style="width: 100%; float: left; margin-left:707px;  '.$showHid_Div.' "><input class="form-control desig_cust" id="temp_desig" placeholder="Other Designation" type="text" name="'.$save_val_txt.'" value="'.$otherVal.'" /></div>
										...
										<div class="form-group">
											...
											<input type="text" name="savvion_key[]" id="Start" value="'.$savvion_key_edit[2].'"  class="datetimepicker-month form-control" placeholder="Start Date" />
											<div class="btn_move"><i class="icon-share-alt"></i></div>
											<input type="text" name="savvion_value[]"  value="'.$savvion_value_edit[2].'"  class="datetimepicker-month form-control" placeholder="Start Date Value" />
											<input type="hidden" name="editkey[]" value="'.$savvion_value_id[2].'" />
										</div>
										<div class="form-group">
											<label for="End">End Date:      </label>
											<input type="text" name="savvion_key[]" id="End" value="'.$savvion_key_edit[3].'"  class="datetimepicker-month form-control" placeholder="End Date" />
											<div class="btn_move"><i class="icon-share-alt"></i></div>
											<input  type="text" name="savvion_value[]"  value="'.$savvion_value_edit[3].'"  class="datetimepicker-month form-control" placeholder="End Date Value" />
											<input type="hidden" name="editkey[]" value="'.$savvion_value_id[3].'" />
										</div>
										<div class="form-group">
								...
								<div class="savvion-health-box2">
									...
									<div class="savvion-health-box2-1">
									   ...
									   <div class="form-group">
											...
											<input type="text" name="savvion_key[]" id="Conferred" value="'.$savvion_key_edit[2].'"  class="datetimepicker-month form-control" placeholder="Conferred Date" />
											<div class="btn_move"><i class="icon-share-alt"></i></div>
											<input type="text" name="savvion_value[]"  value="'.$savvion_value_edit[2].'"  class="datetimepicker-month form-control" placeholder="Conferred Date Value" />
											<input type="hidden" name="editkey[]" value="'.$savvion_value_id[2].'" />
										</div>
										<div class="form-group">
									   ...
									   <div class="form-group">
											...
											<input type="text" name="savvion_key[]" id="From" value="'.$savvion_key_edit[5].'"  class="datetimepicker-month form-control" placeholder="Attended From date" />
											<div class="btn_move"><i class="icon-share-alt"></i></div>
											<input type="text" name="savvion_value[]"  value="'.$savvion_value_edit[5].'"  class="datetimepicker-month form-control" placeholder="Attended From date Value" />
											<input type="hidden" name="editkey[]" value="'.$savvion_value_id[5].'" />
										</div>
									   <div class="form-group">
											<label for="To">Attended To Date:    </label>
											<input type="text" name="savvion_key[]" id="To" value="'.$savvion_key_edit[6].'"  class="datetimepicker-month form-control" placeholder="Attended To Date" />
											<div class="btn_move"><i class="icon-share-alt"></i></div>
											<input type="text" name="savvion_value[]"  value="'.$savvion_value_edit[6].'"  class="datetimepicker-month form-control" placeholder="Attended To Date Value" />
											<input type="hidden" name="editkey[]" value="'.$savvion_value_id[6].'" />
										</div>
	...
	function hide_div(id,typ)
		{
			...
			if(typ=="designation"){
			if(id==17){document.getElementById(\'temp_div\').style.display="";
			jQuery(".desig").attr("name","");
			jQuery(".desig_cust").attr("name","savvion_value[]");
			
			}
			else{document.getElementById(\'temp_div\').style.display="none";
			jQuery(".desig").attr("name","savvion_value[]");
			jQuery(".desig_cust").attr("name","");
			}
			}
	}	
jQuery(document).ready(function(){
	...
	jQuery(".btn_move").click(function(){
		 ...
		 //pts3 = jQuery(this).prev("input[type=hidden]").val();
		 //nts3 = jQuery(this).next("select").val(pts3);
		//jQuery("select.quali option[text=\' + pts + \']").attr("selected","selected");
		/*  jQuery(".quali:select option").filter(function() {
			alert(jQuery(this).text());
    		return jQuery(this).text() == pts; 
		}).attr("selected", true) */
		
		
	
		jQuery("select.quali option").each(function() { this.selected = (this.text == pts); });
		jQuery("select.deg_options option").each(function() { this.selected = (this.text == pts); });
	...
	function approvesavvioncheck(){
		...
		$extra_qc_comments 	= utf8_encode($_REQUEST['extra_qc_comments']); 
		$isAddEdit = $db->updateCol('status,bot_status,extra_qc_comments',"6,1,'$extra_qc_comments'",'savvion_check',"id=$savvion_check_id");
	...
	function rejectsavvion(){
		...
		$isAddEdit = $db->updateCol('status,bot_status,extra_qc_comments',"2,0,'$extra_qc_comments'",'savvion_check',"id=$rejsavvion_check_id");
	...
	function delegateSavvionChecks(){
		...
		if($_REQUEST['ERR']==''){
			foreach($_REQUEST['record'] as $term){
				$uCls = "user_id=$analyst_id,status='1'";
				$isUpdate = $db->update($uCls,"savvion_check","id=$term");
	...
	function getQuali($savvion_check_id,$subBar){
		global $db;
		if (preg_match('/ED/',$subBar)){
		$selRec = $db->select("savvion_check_values","savvion_key","savvion_check_id=$savvion_check_id");
		
		$rec = array();
		while($rsRec = mysql_fetch_array($selRec)){
			
			$rec[] = $rsRec['savvion_key'];
		}
		
		return ($rec[1]!="")?$rec[1]:'N/A';
