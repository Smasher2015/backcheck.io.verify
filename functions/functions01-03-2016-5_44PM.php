...
function client_case_info($twhere='',$torder='',$isSearch=''){
	...
	if($isSearch == 'yes'){
		...
		if($LEVEL==1) $wh= ""; else $wh= " AND com_id=$COMINF[id]";
...
function advance_search_case_info($twhere='',$torder='',$isSearch=''){
	...
	$columns = array("$companyname".$comma,"vd.emp_id","v_name","cc.checks_title","u.first_name","","","v_date","v_status");
	...
	*/
	...
	// ata work
	$cols ="$cols,vd.v_id,  vd.v_name,vd.v_nic,vd.emp_id,vc.as_id,vd.com_id,vd.v_ftname,vd.v_status,vd.v_rlevel,vd.v_sent,v_bmk,v_refid,co.name as com_name, cc.checks_title,concat(u.first_name,' ',u.last_name) as fullname";
// end ata work
	...
	if( !empty($requestData['search']['value']) ) {
		...
		$totalFiltered = @mysql_num_rows($db->select($tbls,"vd.v_id,vc.as_cldate, vc.as_addate","$where GROUP BY  vc.as_id  $having "));
		...
		$total = @mysql_num_rows($db->select($tbls,"vd.v_id,vc.as_cldate, vc.as_addate","$where GROUP BY  vc.as_id  $having "));
		
		
		$cases = $db->select($tbls,$cols,"$where GROUP BY vc.as_id $having ORDER BY $orderBy  LIMIT ".$requestData['start']." ,".$requestData['length']);
		$dCount = @mysql_num_rows($cases);
...
function UpdateCheckStatus($asid,$checkStatus){
	...
	if($checkStatus =='Rejected'){
		if(!empty($comments)){
			...
			$isInsUpd = $db->update("as_qastatus='$checkStatus',t_check=0,as_rej_count='$as_rej_count'","ver_checks","as_id=$asid");
			addActivity('qastatus',"Check Has Been Rejected By QA",0,'',$_REQUEST['case'],$_REQUEST['ascase'],'rejected');
			$uID = $_SESSION['user_id']; 
			...
			// insert qa logs
			$db->insert("as_id,user_id,qa_status","$asid,$uID,'Rejected'",'qa_logs');
			//return $isInsUpd;
	...
	if($checkStatus =='Approved'){
			...
			addActivity('qastatus',"Check has been Approved By QA",0,'',$_REQUEST['case'],$_REQUEST['ascase'],'approved');
			$uID = $_SESSION['user_id']; 
			...
			$isInc = $db->insert($cols,$vals,'comments');
			// insert qa logs
			$db->insert("as_id,user_id,qa_status","$asid,$uID,'Approved'",'qa_logs');
			//return $isInsUpd;
	...
	if($checkStatus =='QA'){
			...
			addActivity('qastatus',"Check has been successfully submit for QA!",0,'',$_REQUEST['case'],$_REQUEST['ascase'],'submitforqa');
...
function getClUser($clientID){
	...
	$cInfo = $db->select("users","*","com_id=$clientID AND is_active=1");
	   ...
	   if ($error == UPLOAD_ERR_OK){
		...
		if(is_numeric($_REQUEST['comid'])){
			...
			//echo "today: ".$tody." enable_time: ".$enable_time; exit;
				...
				$para = "$para,pname='$_REQUEST[pname]',phone='$_REQUEST[phone]',pymterm='$_REQUEST[pterm]',slab_id='$_REQUEST[slab_id]',comment='$_REQUEST[comments]',credits='$credits',mode_of_payment='$_REQUEST[mode_of_payment]',can_download_reports='$can_download_reports',location_wise='$location_wise',monthly_credits_allowed='$monthly_credits_allowed'";
		...
		if(mysql_num_rows($user_info)>0){
			...
			if(mysql_num_rows($company)==0){
				...
				$values = "$values, '$_REQUEST[pterm]','$_REQUEST[comments]','$_REQUEST[pname]','$_REQUEST[phone]','$_REQUEST[cerp]',$paid,'$credits','$_REQUEST[mode_of_payment]','$can_download_reports','$location_wise','$monthly_credits_allowed'";
				$cols   = "name,email,ind_id,type,location,pymterm,comment,pname,phone,erpid,paid,credits,mode_of_payment,can_download_reports,location_wise,monthly_credits_allowed";
	...
	if(is_array($checkInf)){
		...
		if(is_array($tcheck)){
			...
			while($field1 = mysql_fetch_array($fields)){
					if($field1['is_req']==1 && $field1['is_multy']==0){
			...
			if(mysql_num_rows($fields)>0){
				...
				if($error==0){
					if($checkInf['checks_id']==1 && $tcheck['t_check']==0 && $i==1){
							$ch = curl_init();
					   $query_string="roll_num=".(int)$_REQUEST['roll_number']."";
						...
						if($userInfo['level_id']==4){
							$xml=validatelogin($user,trim($_POST['password']),$url);
							 ...
							 if(trim($apidata['RESULT'])=='error'){
									...
									$infoarr['country']='PK';
									$infoarr['phonenumber']=$clientInfo['phone'];
									$infoarr['currency']='1';
...
return $rand_id;
 ...
 // by khl
	...
	function createNotifications($level_id,$a_info="",$v_id='',$notify_to='',$to_email='',$a_type='notification',$as_id=''){
	...
	function getlogo()
 {
  global $db,$COMINF,$LEVEL; 
	...
	global $db;
		...
		while($rsInsuffAtt =	@mysql_fetch_assoc($selInsuffAtt)){
			if($rsInsuffAtt['att_insuff_date']!=""){
				...
				$enable_date = ($rsInsuffAtt['att_suff_date']!="")?$rsInsuffAtt['att_suff_date']:date("Y-m-d");
				$now = strtotime($disable_date); // or your date as well
				$your_date = strtotime($enable_date);
				$datediff =   $your_date - $now;
				$datediff =  floor($datediff/(60*60*24));
	...
	$tbls = "ver_data vd INNER JOIN ver_checks vc ON vd.v_id=vc.v_id INNER JOIN checks ck ON vc.checks_id=ck.checks_id INNER JOIN company co ON vd.com_id=co.id LEFT JOIN uni_info uni ON vc.as_uni=uni.uni_id";
	...
	$cols = "co.name as Client, emp_id AS 'Employee ID',vd.v_name AS 'Candidate Name', ck.checks_title AS 'Check title',
			vc.as_status AS 'Working Status' ,vc.as_vstatus AS 'Verification Status' , DATE(vc.as_date) AS 'Received On', DATE(vc.as_cldate) AS 'Closed On' ,vc.user_id AS 'Analyst Name',vc.as_date as 'Assign Date', vc.as_addate , vc.as_cldate, vd.com_id ".$ifinvoice_select.",uni.uni_Name AS 'IA'";
	...
	if(mysql_field_name($setRec, $i)!='as_cldate' && mysql_field_name($setRec, $i)!='as_addate' && mysql_field_name($setRec, $i)!='com_id' ){
	...
	/*if($key == 8)
	{
		if($rec[9] != '' && $rec[9] != '0000-00-00 00:00:00')
		{
		$days  = getDaysFromDates($rec[9],$rec[8],str_replace('"','',(int)$rec['com_id']) );
		}
		else
		{
		$days  = getDaysFromDates($today,$rec[8],str_replace('"','',(int)$rec['com_id']) );
			
		//$days  = getDaysFromDates($today,$rec['as_addate'],$rec['com_id']);
		}
		 $value = $days;
	}
	*/
	...
	$wwhr = ($owhere=="")?"":" $owhere AND ";
	$where = "$wwhr v_isdlt=0 $whr $excludeComs $excludeUsers";
	$tbls = "ver_data vd INNER JOIN ver_checks vc ON vd.v_id=vc.v_id INNER JOIN checks ck ON vc.checks_id=ck.checks_id INNER JOIN company co ON vd.com_id=co.id INNER JOIN uni_info uni ON vc.as_uni=uni.uni_id";
	if($bycase) $GROUPBY = "GROUP BY vd.v_id"; else $GROUPBY="GROUP BY vc.as_id";
	$cols = "co.name as Client, emp_id AS 'Employee ID',vd.v_name AS 'Candidate Name', ck.checks_title AS 'Check title',
			vc.as_status AS 'Working Status' ,vc.as_vstatus AS 'Verification Status' , DATE(vc.as_date) AS 'Received On', DATE(vc.as_cldate) AS 'Closed On' ,vc.user_id AS 'Analyst Name',vc.as_date as 'Assign Date', vc.as_addate , vc.as_cldate, vd.com_id ".$ifinvoice_select.",uni.uni_Name AS 'IA'";
	//echo "select $cols FROM $tbls WHERE $where $GROUPBY ORDER BY $ORDER  $limit"; exit;
...
*/	
	...
	$v_id = (int)$_POST['v_id'];
	$att_upd_date_val = ',CURRENT_TIMESTAMP';
	$countInsuff = 0;
	if(count($att_ids)>0){
		...
		$insuf_comment_col = ',att_comments';
		$insuf_comment_val = $_POST['insuf_comm_'.$att_id];
		
		$comment_val = ","."'$insuf_comment_val'";
		...
		// echo "UPDATE attachments SET att_insuff=$att_insuff_status $att_upd_date_col = $att_upd_date_val $insuf_comment_col='".$insuf_comment_val."' WHERE att_id=".$att_id." <br>";
		 $db->updateCol("att_insuff $att_upd_date_col $insuf_comment_col","$att_insuff_status $att_upd_date_val $comment_val","attachments","att_id=".$att_id);
		}
		}  
//var_dump($countInsuff); die;
	if($countInsuff>0){
	$Att = ($countInsuff>1)?'Attachments':'Attachment';
	$a_info = "$countInsuff $Att marked as insufficient by analyst (".$_SESSION['first_name'].") of Check:$_POST[chk_name] (Client: $_POST[com_name] and Candidate: $_POST[app_name])";
					
	 $notify = createNotifications(4,$a_info,$v_id,'','','insufficient',$as_id);
	}
