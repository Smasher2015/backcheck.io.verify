...
$ch = curl_init();
 $query_string="action=lead_add&pams[CREATED_BY_ID]=479&pams[TITLE]=".urlencode($comp_arr['name'])."&pams[NAME]=".urlencode($comp_arr['name'])."&pams[COMMENTS]=".urlencode($comp_arr['comment'])."&pams[EMAIL_WORK]=".$comp_arr['email']."&pams[PHONE_WORK]=".$comp_arr['phone']."&pams[SOURCE_ID]=".urlencode("Verification System")."&pams[Erp Id]=".$comp_arr['erpid']."";
...
function add_task($task_arr,$parent_id=0){
 $ch = curl_init();
 $enddateplan=getdatedifference(date("Y-m-d"),12);
 $deadline=getdatedifference(date("Y-m-d"),15);
 $remainderdate=getdatedifference(date("Y-m-d"),2);
 $query_string="action=task_add&CREATED_BY=507&task_name=".$task_arr['task_name']."&desc=".$task_arr['task_desc']."&time_estimate=2&PARENT_ID=$parent_id&user_id=".$task_arr['user_id']."&group_id=".$task_arr['group_id']."&START_DATE_PLAN=".date("Y-m-d")."&END_DATE_PLAN=".$enddateplan."&DEADLINE=$deadline&remainderdate=$remainderdate";
...
function clientulkupload(){
			global $db,$COMINF,$LEVEL; // print_r($_REQUEST);
			if($LEVEL!=4){
			...
			$order_id = ChecksAmount($_REQUEST,$company_id);
			...
			if($_REQUEST['ERR']==''){
				...
				while(isset($_REQUEST['ename'.$nums])){
					...
					$cols="thum,image,v_title,v_gender, v_first_name,v_last_name,v_name, v_ftname,v_nic,v_dob,com_id,v_recdate,v_bcode,v_uadd, emp_id";
					$values="'$thum','$image','$etitle','$gender','$first_ename','$last_ename','$ename','$fname','$cnic','$dob',$company_id,'$recdate','$bCode',$uID, '".$empcode."'";
					//echo "cols: ".$cols." values: ".$values; exit;
					$lead_array=array();
					$lead_array['name']='Case For '.$ename ." - $bCode - Total Checks: ".count($_REQUEST['ischeck'.$nums]);
					...
					// khl	//		
								...
								$bitrixuserid2=$bitrixuserid['bitrix_uid'];
								$userid2=$bitrixuserid['user_id'];
		...
		$data = $db->select("comp_info","*","id = ".$_REQUEST['select_company']."");
		$companyinfo = mysql_fetch_array($data);
		$cname = $companyinfo['cname'];
							
  		
			$cols="as_id,d_type,d_mtitle, d_stitle, d_num, d_value, user_id, d_isdlt";
			$values="'$CID','multy','select_company', '".$_REQUEST['select_company']."', '1', 'yes', '".$_SESSION['user_id']."', '0'";
								 
								 $db->insert($cols,$values,"add_data");
								 
			$cols="as_id,d_type,d_mtitle, d_stitle, d_num, d_value, user_id, d_isdlt";
			$values="'$CID','multy','job_title', '".$_REQUEST['job_title']."', '1', 'yes', '".$_SESSION['user_id']."', '0'";
								 
								 $db->insert($cols,$values,"add_data");
			$cols="as_id,d_type,d_mtitle, d_stitle, d_num, d_value, user_id, d_isdlt";
			$values="'$CID','multy','behaviour', '".$_REQUEST['behaviour']."', '1', 'yes', '".$_SESSION['user_id']."', '0'";
								 
								 $db->insert($cols,$values,"add_data");
			$cols="as_id,d_type,d_mtitle, d_stitle, d_num, d_value, user_id, d_isdlt";
			$values="'$CID','multy','salary', '".$_REQUEST['salary']."', '1', 'yes', '".$_SESSION['user_id']."', '0'";
								 
								 $db->insert($cols,$values,"add_data");
			
			$cols="as_id,d_type,d_mtitle, d_stitle, d_num, d_value, user_id, d_isdlt";
			$values="'$CID','multy','job_duration', '".$_REQUEST['job_duration']."', '1', 'yes', '".$_SESSION['user_id']."', '0'";
  ...
  <tr>
  	...
  	<td align=\"center\" width=\"100%\" colspan=\"3\"><h3 style=\"font-weight:normal; color:#70bff3;\">Dear ".$cname." ,</h3></td>
  </tr>
  <tr>
...
$message.="<tr>
	...
	<td > ".$_REQUEST['job_title']."</td>
  	...
  	<td >Behaviour</td>
	<td > ".$_REQUEST['behaviour']."</td>
   </tr><tr>
  	<td >Salary</td>
	<td > ".$_REQUEST['salary']."</td>
   </tr><tr>
  	<td >Job Duration</td>
	<td > ".$_REQUEST['job_duration']."</td>
   </tr>";

 		 $message .= "<tr>
  	<td align=\"left\" width=\"40%\" colspan=\"3\">Please verify this information <a href=".SURL."preemp_verification_inc.php?id=".base64_encode($CID).">Click Here</a></td>
  </tr>";
 ...
 $message .= " <tr>
   ...
   */
   $mail->Subject    = "Previous Employment";
  ...
  //echo $message.'<br>'; exit;
   ...
   $mail->Send();
					...
					//createNotifications(4,$a_info,'',$analyst_id);
								...
								Attachments :
								".implode(",",$attachments)."";
								$task_array['user_id']=$bitrixuserid2;
								$task_array['group_id']=$resRoles['group_id'];
				...
				if($VID){	
					...
					$uptorderAmount = 0;
						...
						foreach($_REQUEST['checks1'] as $check){
							...
							$cols="as_bcode,v_id,checks_id,as_uadd,as_addate, as_date, as_cost2";
							$values="'$bCode',$VID,$chk,$uID,CURRENT_TIMESTAMP, CURRENT_TIMESTAMP,'$as_cost2'";
							if($db->insert($cols,$values,"ver_checks")){
								$CID=$db->insertedID;
...
function advance_search_case_info($twhere='',$torder='',$isSearch=''){
	...
	$daysCol = ", (DATEDIFF('$today', DATE($check_date))-((WEEK($check_date) - WEEK('$today')) * 2) - (CASE WHEN WEEKDAY($check_date) = 6 THEN 1 ELSE 0 END) - (CASE WHEN WEEKDAY('$today') = 5 THEN 1 ELSE 0 END)) as days ";
	...
	$cols = "COUNT(vd.v_id) AS cnt,DATE_FORMAT(v_date,'%d-%b-%Y') AS ndate,vd.v_crd,vd.v_stdate,vd.v_date,vd.v_cldate,vd.emp_id, DATE(vc.as_addate) as add_date, vd.com_id, vc.as_cldate, vc.as_addate $daysCol ";
	...
	$tbls = "ver_data vd INNER JOIN ver_checks vc ON vd.v_id=vc.v_id INNER JOIN company co ON vd.com_id=co.id"; 
	...
	if( !empty($requestData['search']['value']) ) {
		...
		$totalFiltered = @mysql_num_rows($db->select($tbls,"vd.v_id,vc.as_cldate, vc.as_addate","$where GROUP BY vd.v_id $having "));
		
	}else{
		$totalFiltered = @mysql_num_rows($db->select($tbls,"vd.v_id,vc.as_cldate, vc.as_addate","$where GROUP BY vd.v_id $having "));
		
	}
		//echo "select  $cols  from $tbls  where $where GROUP BY vd.v_id $having ORDER BY $orderBy LIMIT ".$requestData['start']." ,".$requestData['length']."";
		
		$total = @mysql_num_rows($db->select($tbls,"vd.v_id,vc.as_cldate, vc.as_addate","$where GROUP BY vd.v_id $having "));
		
		
		$cases = $db->select($tbls,$cols,"$where GROUP BY vd.v_id $having ORDER BY $orderBy  LIMIT ".$requestData['start']." ,".$requestData['length']);
		$dCount = @mysql_num_rows($cases);
...
function UpdateCheckStatus($asid,$checkStatus){
	...
	if($checkStatus =='Rejected'){
		if(!empty($comments)){
			$isInsUpd = $db->update("as_qastatus='$checkStatus',t_check=0","ver_checks","as_id=$asid");
	...
	if(isset($_REQUEST['checks']) && is_array($_REQUEST['checks'])){
		...
		foreach($_REQUEST['checks'] as $check){
			if(is_numeric($_REQUEST["cost$check"])){
				...
				$cols = "dd_bcode,dd_vdcost,dd_uni,dd_bene,dd_fee,dd_units,dd_cdate,dd_status,dd_user,dd_att1,dd_tit1";
				$vals = "'$as_bcode',$vcost,$uni,'$ben',$fee,1,CURRENT_TIMESTAMP,1,$uID,'$attch','$fname'";
				
				
				$selBBCode = $db->select("dd_data","dd_id","dd_bcode='$as_bcode'");
...
function adddemanddraft(){
	...
	$selBBCode = $db->select("dd_data","dd_id","dd_bcode='$_POST[bcode]'");
	$numRows = mysql_num_rows($selBBCode);
	//echo $numRows;  exit;
	if($numRows>0){
...
function register_(){
		...
		if($_POST['passa'] != $_POST['passb']){
			...
			$passVal = "";
				...
				$cols ="first_name,last_name,email,username, $passField level_id,kpi";
				...
				$vals = "'$_POST[fname]','$_POST[lname]','$_POST[email]','$_POST[email]', $passVal $_POST[ulevel],'$_POST[kpi]'";
	   ...
	   if ($error == UPLOAD_ERR_OK){
		...
		if(is_numeric($_REQUEST['comid'])){
			...
			//echo "today: ".$tody." enable_time: ".$enable_time; exit;
				...
				$para = "$para,pname='$_REQUEST[pname]',phone='$_REQUEST[phone]',pymterm='$_REQUEST[pterm]',slab_id='$_REQUEST[slab_id]',comment='$_REQUEST[comments]',credits='$credits',mode_of_payment='$_REQUEST[mode_of_payment]',can_download_reports='$can_download_reports',monthly_credits_allowed='$monthly_credits_allowed'";
		...
		if(mysql_num_rows($user_info)>0){
			...
			if(mysql_num_rows($company)==0){
				...
				$values = "$values, '$_REQUEST[pterm]','$_REQUEST[comments]','$_REQUEST[pname]','$_REQUEST[phone]','$_REQUEST[cerp]',$paid,'$credits','$_REQUEST[mode_of_payment]','$can_download_reports','$monthly_credits_allowed'";
				$cols   = "name,email,ind_id,type,location,pymterm,comment,pname,phone,erpid,paid,credits,mode_of_payment,can_download_reports,monthly_credits_allowed";
						...
						if($value!=''){
							...
							if($field['is_multy']==0){
								...
								if($dCnt>0){
									...
									if($isInsUpd){
										if($tcheck['checks_id']==1 && $tcheck['t_check']==0 && $i==1){
	...
	if($input['in_type'] == "radio")
			...
			switch($input['in_name']){
				...
				case'select':
						...
						$select = "<select $clkOp title=\"$field[fl_title]\" class=\"input form-control $isReq\" name=\"$field[fl_key]$asID\" >";
...
return $rand_id;
 ...
 // by khl
	...
	function checkEmpId($emp_id,$echo=0,$company_id=0){
		...
		if($r>0){
			...
			$msg =  "This Employee Code already exists <a href='?action=details&case=$rs[v_id]&_pid=20' target='_blank' >$rs[v_name]</a>"; 
		}else{
			$msg = "not-found"; 
		}
	...
	function createNotifications($level_id,$a_info="",$v_id='',$notify_to='',$to_email=''){
	...
	function getlogo()
 {
  global $db,$COMINF,$LEVEL; 
	...
	function getTax($id=1,$country_id=0){
		global $db;
		$wher = ($country_id!=0)?"country_id=$country_id":"id=$id";
	...
	function getDaysFromDates($as_cldate,$as_addate,$com_id=0){
	...
	function enableDisableClient($com_id,$disabled_id){
			...
			emailTmp( $NotificationText,$NotificationSubject,'khalique.ahmed3@gmail.com','','','','',$userNames."[$lev]");
			}
	...
	function lastSixMonthChecks($checkStatus='', $verificationStatus=''){
		...
		if($verificationStatus!=''){
			$LastSixMonthOpenCheckWhere 	= "vc.as_addate >= DATE_FORMAT(CURDATE(), '%Y-%m-01') - INTERVAL 6 MONTH AND vd.com_id=$COMINF[id] AND vc.`as_status`='$checkStatus' AND  vc.`as_vstatus`='$verificationStatus'";
		}else{
			$LastSixMonthOpenCheckWhere 	= "vc.as_addate >= DATE_FORMAT(CURDATE(), '%Y-%m-01') - INTERVAL 6 MONTH AND vd.com_id=$COMINF[id] AND vc.`as_status`='$checkStatus' ";
		}
	...
	function SingleMonthChecks($checkStatus='', $verificationStatus='',$month=0, $year=0){
		...
		$LastSixMonthOpenCheckCols		= "COUNT(vc.`as_status`) Applicant_Check_Status"; 
		if($verificationStatus!=''){
			$LastSixMonthOpenCheckWhere 	= "MONTH(vc.as_addate) = $month AND YEAR(vc.as_addate) = $year  AND vd.com_id=$COMINF[id] AND vc.`as_status`='$checkStatus' AND  vc.`as_vstatus`='$verificationStatus'";
		}else{
			$LastSixMonthOpenCheckWhere 	= "MONTH(vc.as_addate) = $month AND YEAR(vc.as_addate) = $year  AND vd.com_id=$COMINF[id] AND vc.`as_status`='$checkStatus' ";
		}
		$LastSixMonthOpenCheckQuery 	= $db->select($LastSixMonthOpenCheckTables,$LastSixMonthOpenCheckCols,$LastSixMonthOpenCheckWhere);
		$LastSixMonthOpenCheckResult 	= mysql_fetch_assoc($LastSixMonthOpenCheckQuery);
		$LastSixMonthOpenCheck 			= $LastSixMonthOpenCheckResult['Applicant_Check_Status'];
	...
	$where = "$wwhr v_isdlt=0 $whr $excludeComs $excludeUsers";
	$tbls = "ver_data vd INNER JOIN ver_checks vc ON vd.v_id=vc.v_id INNER JOIN checks ck ON vc.checks_id=ck.checks_id INNER JOIN company co ON vd.com_id=co.id";
	...
	$cols = "co.name as Client, emp_id AS 'Employee ID',vd.v_name AS 'Candidate Name', ck.checks_title AS 'Check title',
			vc.as_status AS 'Working Status' ,vc.as_vstatus AS 'Verification Status' , DATE(vc.as_date) AS 'Received On', DATE(vc.as_cldate) AS 'Closed On' , vc.as_addate, vc.as_cldate";
	//echo "select $cols FROM $tbls WHERE $where $GROUPBY ORDER BY $ORDER  $limit"; exit;
	$setRec = $db->select($tbls,$cols,"$where $GROUPBY $having ORDER BY $ORDER  $limit");	
	...
	if(mysql_field_name($setRec, $i)!='as_addate' && mysql_field_name($setRec, $i)!='as_cldate'){
	...
	$s_checks_id  	= $posted['s_checks_id'];
	$user_id  		= (int) $posted['user_id'];
	$com_id  	= (int) $posted['client_id'];
...
*/	
	$whr = " AND level_id='$LEVEL' AND  user_id='$user_id'";// 
		$where = "com_id='$com_id' $whr";
	...
	$vals = "$LEVEL,'$from_dt','$to_dt','$check_status','$candidate_name_id','$s_checks_id','$com_id','$_days','$user_id', $send_weekly_update";
	 ...
	 $db->updateCol($cols,$vals,"clients_saved_search","id=$res[id]");
	...
	function inviteApplicantVerify(){
		...
		if($error==0){	
			...
			$c++;
				...
				//die;
				msg('sec',"Invitation Successfylly Sent To Applicant(s)");
				//return $usersEmail;
	...
	function addApplicantVerify(){
		...
		if($error==0){	
			if($q==1){
				...
				msg('sec',"Applicant(s) Successfully Added...");
			}
			return $usersEmail;
		}
	...
	function checkUser($email,$first_name,$last_name='',$employeCode=0){
		global $db,$COMINF;
		//echo $email,die;
		$company_id = $COMINF['id'];
	...
	function verifyApplicants(){
		...
		$employeCodes			= $_REQUEST['employeCodes'];
			...
			//die;
			msg('sec',"Request Successfylly Sent To Applicant(s)");
	}
	...
	// by khl
	function ChecksAmount($data,$com_id,$isBulk=false){
	...
	// add order
	$inserted_id = addClientInvoice($com_id,$totalAmount);
	if(!($inserted_id)){
	if(!$isBulk){
	...
	if(!$isBulk){
	msg('err',"Your order amount $totalAmount exceeded  from your credits $clientCredits. Please contact support team. ");
	}else{
	echo  "Your order amount $totalAmount exceeded  from your credits $clientCredits. Please contact support team. "; exit;	
	}
	}
	//die;
	}
	...
	if(count($att_ids)>0){
		...
		$insuff_title = ($att_insuff_status==0)?'insufficient':'sufficient';
		//echo "UPDATE attachments SET att_insuff='".$att_insuff_status."' WHERE att_id=".$att_id." <br>";
		$db->updateCol("att_insuff",$att_insuff_status,"attachments","att_id=".$att_id);
		}
		}
		
	$a_info = "Attachments marked as sufficient / insufficient by analyst (".$_SESSION['first_name'].") of Check:$_POST[chk_name] (Client: $_POST[com_name] and Candidate: $_POST[app_name])";
