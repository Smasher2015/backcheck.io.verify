...
$bitrix_admin_id = ($comp_arr['country_id']!=171)?529:507; 
...
function add_task($task_arr,$parent_id=0){
 $ch = curl_init();
 // bitrix admin: 480=Sadia 507=Saima 529=Sharjeel
 $bitrix_admin_id = ($task_arr['country_id']!=171)?529:507;
...
function weekly_checks_data($groupBy=""){
	...
	if($checkStatus =='Rejected'){
		...
		if(!empty($comments)){
			...
			$isInsUpd = $db->update("as_qastatus='$checkStatus',as_vstatus='Work In Progress',as_status='Open',t_check=0,as_rej_count='$as_rej_count'","ver_checks","as_id=$asid");
	...
	function addInvoiceNumbers($checks=array(),$cost=array(),$invoice_number,$is_tax=0,$invoice_date=''){
		...
		if(count($checks)==0){
			...
			foreach ($checks as $check){
				...
				$TaxAmount = ($vCheck['citystate_id']!=0 && $vCheck['citystate_id']!='')?getTax(0,0,$vCheck['citystate_id']):getTax(0,$vCheck['country_id'],0);
				$where = "as_id=".$check;				
