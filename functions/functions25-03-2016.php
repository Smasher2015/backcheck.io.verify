...
$ch = curl_init();
// bitrix admin: 480=Sadia 507=Saima
$bitrix_admin_id = ($comp_arr['country_id']!=171)?480:507; 
...
function add_task($task_arr,$parent_id=0){
 $ch = curl_init();
 // bitrix admin: 480=Sadia 507=Saima
 $bitrix_admin_id = ($task_arr['country_id']!=171)?480:507;
...
function clientulkupload(){
			...
			if($_REQUEST['ERR']==''){
				...
				while(isset($_REQUEST['ename'.$nums])){
					...
					$cols="thum,image,v_country,v_title,v_gender, v_first_name,v_last_name,v_name, v_ftname,v_nic,v_dob,com_id,v_recdate,v_uadd, emp_id";
					$values="'$thum','$image','$country','$etitle','$gender','$first_ename','$last_ename','$ename','$fname','$cnic','$dob',$company_id,'$recdate',$uID, '".$empcode."'";
...
function addMoreChecks(){
		...
		if($_REQUEST['ERR']==''){
			...
			$nums = 1;
				...
				// insert notificaification to manager about new cases
				$a_info = "More Checks added by ".$_SESSION['fname']." ( $comInfo[name] ) against case id #$VID";
...
function addMoreAttachments(){
		...
		if($_REQUEST['ERR']==''){
			...
			$nums = 1;
				...
				// insert notificaification to manager about new cases
				$a_info = "More attachments added by ".$_SESSION['fname']." ( $comInfo[name] ) against case id #$VID and check id #$CID";
	   ...
	   if ($error == UPLOAD_ERR_OK){
		...
		if(mysql_num_rows($user_info)>0){
			...
			if(mysql_num_rows($company)==0){
				...
				$values = "$values, '$_REQUEST[pterm]','$_REQUEST[comments]','$_REQUEST[pname]','$_REQUEST[phone]','$_REQUEST[cerp]',$paid,'$credits','$_REQUEST[mode_of_payment]','$_REQUEST[account_type]','$_REQUEST[is_check_wise_pay]','$can_download_reports','$location_wise','$monthly_credits_allowed'";
				$cols   = "name,email,ind_id,type,location,pymterm,comment,pname,phone,erpid,paid,credits,mode_of_payment,account_type,is_check_wise_pay,can_download_reports,location_wise,monthly_credits_allowed";
...
function updateCheck($verID,$uCols){
	...
	$a_info = "Check Report downloaded  by ".$_SESSION['fname']." ( $comInfo[name] ) of check id # $verID";
...
function updateData($vID,$uCols){
	...
	$a_info = "Case Report downloaded  by ".$_SESSION['fname']." ( $comInfo[name] ) of case id # $vID";
	$notify = createNotifications(4,$a_info);
...
//var_dump($countInsuff); die;
	...
	$a_info = "$countInsuff $Att marked as insufficient by analyst (".SUDONYMS.") of Check:$_POST[chk_name] (Client: $_POST[com_name] and Candidate: $_POST[app_name])";
	...
	$a_info = "$_POST[chk_name] Check marked as sufficient by analyst (".SUDONYMS.") of Candidate $_POST[app_name]";
