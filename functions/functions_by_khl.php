...
//new
	...
	function clientulkupload(){
			...
			if($_REQUEST['ERR']==''){
				...
				while(isset($_REQUEST['ename'.$nums])){
					...
					$values="'$thum','$image','$ename','$fname','$cnic','$dob',$COMINF[id],'$recdate','$bCode',$uID, '".$empcode."'";
					...
					$a_info = ($_REQUEST['is_bulk']==1) ? "New Cases added with Bulk Uploaded by ".$_SESSION['fname']." ( $COMINF[name] ) " : "New Case Added by ".$_SESSION['fname']." ( $COMINF[name] ) ";
					
					$notify = createNotifications(4,$a_info);
					
					if(!$any_error && $_REQUEST['ERR']=='') msg("sec","Records added successfully...");			
			}
	}
	...
	function clientBulkUploadAjax(){
			...
			if($_REQUEST['ERR']==''){
				...
				while(isset($_REQUEST['ename'.$nums2])){
					...
					if($fname==''){
					echo "Please type applicant's father name !"; exit;
					$isErr = 1;
					}
					if($cnic==''){
					echo "Please type CNIC !"; exit;
					$isErr = 1;
					}
					
					$checkCnic = checkCnic($cnic);
					
					if($checkCnic != 'not-found'){
					echo "CNIC already exists !"; exit;
					$isErr = 1;
					
					}
					$checkEmpId = checkEmpId($empcode);
					if($checkEmpId != 'not-found'){
					echo "Employee Code already exists !"; exit;
					$isErr = 1;
					}
					...
					$duplicatesCnic = find_duplicates($listOfCnic,'CNIC');
					$duplicatesEmp = find_duplicates($listOfEmp,'Employee Code');																				
					if($duplicatesCnic){
				...
				while(isset($_REQUEST['ename'.$nums])){
					...
					//createNotifications(4,$a_info,'',$analyst_id);
						 ...
						 echo "Case [$ename] insertion error!"; 
					}
						$nums++;
					}
				}
					...
					if(!$any_error && $_REQUEST['ERR']=='') echo "Records added successfully..."; exit;			
				}
			}
	}
	...
	function find_duplicates($arr,$title='CNIC') {
	...
	foreach ($new_array as $rec => $n) {
		$msg .= "Duplicate entries of $title: <br />". $rec;
		if($n > 1)
			$msg .= " ($n)";
		$msg .= "<br />";
	}
...
function addMoreChecks(){
		...
		if($_REQUEST['ERR']==''){
			...
			$nums = 1;
				...
				// insert notificaification to manager about new cases
				$a_info = "More Checks added by ".$_SESSION['fname']." ( $COMINF[name] ) against case id $VID";
...
function addMoreAttachments(){
		...
		if($_REQUEST['ERR']==''){
			...
			$nums = 1;
				...
				// insert notificaification to manager about new cases
				$a_info = "More attachments added by ".$_SESSION['fname']." ( $COMINF[name] ) against case id $VID and check id $CID";
...
function client_checks_info($owhere,$limit='',$bycase=false){
	...
	$cols = "d.image,d.v_name,ck.checks_title,c.as_stdate,d.v_id,c.as_id,c.as_status,c.as_vstatus,c.as_remarks,c.as_pdate,c.as_date";
...
function client_case_info($twhere='',$torder='',$isSearch=''){
	...
	//echo "select  $cols  from $tbls  $where GROUP BY d.v_id ORDER BY $order";
...
function weekly_checks_data($groupBy=""){
	...
	"vc.as_id,vc.v_id,cmp.id AS com_ids, chk.checks_title,vd.v_name, vc.as_status, vd.emp_id ","vc.as_pdate >= CURDATE() - INTERVAL DAYOFWEEK(CURDATE())+6 DAY
	AND vc.as_pdate < CURDATE() - INTERVAL DAYOFWEEK(CURDATE())-1 DAY $groupBy"
	);
...
function UpdateCheckStatus($asid,$checkStatus){
	...
	$db = new DB();
	$comments = $_REQUEST['rej_comments'];
	if($checkStatus =='Rejected'){
		if(!empty($comments)){
			$isInsUpd = $db->update("as_qastatus='$checkStatus'","ver_checks","as_id=$asid");
			
			$uID = $_SESSION['user_id']; 
			
			$cols = "p_id,_id,com_text,com_type,user_id,clent_id";
			
			$vals = "0,$asid,'$_REQUEST[rej_comments]','qa',$uID,$_REQUEST[analyst_id]";
			
			$isInc = $db->insert($cols,$vals,'comments');
			//return $isInsUpd;
			msg("sec","Check has been rejected!");
		}else{
			msg('err',"QA comments must have some value!");
		}
	...
	if($checkStatus =='Approved'){
			...
			$vals = "0,$asid,'$_REQUEST[app_comments]','qa',$uID,$_REQUEST[analyst_id]";
			
			$isInc = $db->insert($cols,$vals,'comments');
			//return $isInsUpd;
			msg("sec","Check has been Approved!");
	}
	if($checkStatus =='QA'){
			$isInsUpd = $db->update("as_qastatus='$checkStatus'","ver_checks","as_id=$asid");
			...
			$isInc = $db->insert($cols,$vals,'comments');
			//return $isInsUpd;
			msg("sec","Check has been successfully submit for QA!");
	}
}
...
function updateCheck($verID,$uCols){
	...
	$a_info = "Report downloaded  by ".$_SESSION['fname']." ( $COMINF[name] ) of check id # $verID";
	$notify = createNotifications(4,$a_info);
...
function caseStatus($asCase,$CaseStatus){
	...
	if($CaseStatus == 'Close'){
		$isInsUpd = $db->update("as_status='$CaseStatus',as_qastatus='QA'","ver_checks","as_id=$asCase");
	}else{
...
function reportCheckDetails($vid,$checkid='',$exCol=''){
	...
	$checksInfo = $db->select($tbls,"DISTINCT *","$where AND as_isdlt=0 AND vd.com_id=$COMINF[id] ORDER BY checks_title");
				...
				if($userInfo['is_active'] == 1){ 
					if(is_numeric($userInfo['com_id'])){
						...
						if(mysql_num_rows($clientInfo) > 0){
							return msg('err','Systems access2 temporary disabled due to non payment!');
						}
					}
...
function emailTmp($table,$title,$sEmail,$fEmial='',$cMail=''){
  ...
  <tr>
    <td align=\"left\" style=\"padding:20px;\" bgcolor=\"#333333\"><img src=\"".SURL."/images/logo_email.png\" width=\"270\" /></td>
    <td align=\"left\" width=\"36%\" style=\"padding:20px;\" bgcolor=\"#333333\"><p style=\"  background-color: #444; padding: 12px 8px; line-height: 1; color: #fff; font-size: 14px; margin: 0; border-radius: 4px; text-align: center;\">Verification System</p></td>
  </tr>
  
  <tr>
  	<td align=\"center\" width=\"100%\" colspan=\"3\"><h2 style=\"color:#465059; padding:20px 0 0 0; margin:0;\">Welcome to Risk Discovered</h2></td>
  </tr>
  
  <tr>
  	<td align=\"center\" width=\"100%\" colspan=\"3\"><img src=\"".SURL."/images/user_icon.png\" width=\"64\" height=\"64\" alt=\"Your Profile Picture\"></td>
  </tr>
  
  <tr>
  	<td align=\"center\" width=\"100%\" colspan=\"3\"><h3 style=\"font-weight:normal; color:#70bff3;\">Hello ".$username."</h3></td>
  </tr>
 
 	
  <tr>
  	<td bgcolor=\"#FFFFFF\" align=\"center\" width=\"100%\" colspan=\"3\" style=\"padding:20px 10px 50px 10px;\">$table</td>
  </tr>
  
  
  
  <tr>
  	<td align=\"center\" width=\"100%\" colspan=\"3\" bgcolor=\"#FFFFFF\"><p style=\"padding:5px 20px; color: #54565c; font-size:13px;\">If you need help or have any questions, please visit our <a href=\"#\" style=\"color:#fd4f00\"><span>Support</span></a>.</p></td>
  </tr>
  
   <tr>
  	<td align=\"center\" width=\"100%\" colspan=\"3\" bgcolor=\"#555555\"><p style=\"padding:5px 20px; color: #ffffff; font-size:13px;\"> &copy; 2007 - 2015 - All rights reserved | Powered by <a href=\"#\" style=\"color:#ffffff\">Background Check Pte Ltd</a> 
</td>
  </tr> 
...
return $rand_id;
 ...
 // by khl
	...
	function checkCnic($cnic){
		...
		$where = "v_nic='".$cnic."' AND com_id='".$COMINF['id']."' ";
		$Q = $db->select("ver_data","v_id",$where); 
		$r = mysql_num_rows($Q);
		
		if($r>0){
			$msg =   "This CNIC already exists"; exit;
		}else{
			$msg =  "not-found"; exit;
		}
				 return $msg; 
	...
	function checkEmpId($emp_id){
		...
		$where = "emp_id='".$emp_id."' AND com_id='".$COMINF['id']."' ";
		
		$Q = $db->select("ver_data","v_id",$where); 
		$r = mysql_num_rows($Q);
		
		if($r>0){						
			$msg =  "This Employee Code already exists"; exit;
		}else{
			$msg = "not-found"; exit;
		}
	...
	function saveFilter($filter_what,$filter_by){
		global $db,$COMINF;
		$uID = $_SESSION['user_id'];
		$where = "filter_what='".$filter_what."' AND com_id=".$COMINF['id']." AND user_id=".$uID." ";
		...
		if($rows>=1){
				...
				$values="$COMINF[id],$uID ,'$filter_what','$filter_by'";
	...
	function getFiltersBy($filter_what, $cols_date = ''){
		global $db,$COMINF;
		$uID = $_SESSION['user_id'];
		$data = array();
		$data['filter_by'] = '';
	...
	function createNotifications($level_id,$a_info="",$v_id='',$notify_to=''){
		...
		$notifyVals = "$uID,'".$v_id."','".$notify_to."','".$a_info."','notification','".$level_id."'";
		...
		if($note){
			...
			$subject = "New Notification";
				...
				//$userInfo = getUserInfo($to_msg);
				$to_email = 'ayaz@xcluesiv.com'; // Manager only;
				
				// for now only to manager 11-06-2015
				if($LEVEL==4){ 
				emailTmp($table,$subject,$to_email);		
				 }
			return true;
		}else{
	...
	function getlogo()
 {
  global $db,$COMINF,$LEVEL; 
	...
	function addInvoiceNumbers($checks=array(),$invoice_number,$is_tax=0){
		global $db,$LEVEL;
		...
		if(count($checks)==0){
			...
			foreach ($checks as $check){
				...
				$where = "as_id=".$check;				
				$values="invoiced=1,invoice_number='".$invoice_number."', invoiced_date=CURRENT_TIMESTAMP , 
				is_tax=".$is_tax.", tax='".getTax()."'";
	...
	function getdatedifference($startdate, $days, $remove_dates=array())
	{
		global $db;
		$skipDates1 = array();
		$skipDates2 = array();
		$selholydays = $db->select("holiday_master","*","");
			while($holydays =	mysql_fetch_array($selholydays)){
				...
				if($datediff>0){
					...
					$skipDates2[] = date(strtotime($hol_sdate . "+$x day"));
					}
				}
		...
		for($i=0; $i<$days; $i++)
		{
			...
			if($nextDay == 0 || $nextDay == 6 || in_array($nextDate,$skipDates1) || in_array($nextDate,$skipDates2))
			{
				//echo $nextDay."<br>";
				$i--;
			}else{ 
	...
	function getDaysFromDates($as_cldate,$as_addate){
	...
	$ClientRefNo 			= $_REQUEST['ClientRefNo']; 
	$DateOfBirth 			= $_REQUEST['DateOfBirth']; 
	$PassportNo 			= $_REQUEST['PassportNo']; 
	$AKAName 				= $_REQUEST['AKAName']; 
	$Gender 				= $_REQUEST['Gender']; 
	$Nationality 			= $_REQUEST['Nationality']; 
	$ArabicName 			= $_REQUEST['ArabicName']; 
	$_UNBOUND_textArea3 	= $_REQUEST['_UNBOUND_textArea3']; 
	$HistoryRemarks 		= $_REQUEST['HistoryRemarks']; 
	$_UNBOUND_historyNew 	= $_REQUEST['_UNBOUND_historyNew']; 
	
	
	
	$Education_UniversityName 		= $_REQUEST['Education_UniversityName'];
	$Education_UniversityAddress 	= $_REQUEST['Education_UniversityAddress'];
	$Education_City 				= $_REQUEST['Education_City'];
	$Education_Country 				= $_REQUEST['Education_Country'];
	$Education_Telephone 			= $_REQUEST['Education_Telephone'];
	...
	$IA_IsFake 						= $_REQUEST['IA_IsFake'];
	$IA_IsOnline 					= $_REQUEST['IA_IsOnline'];
	$IA_Telephone 					= $_REQUEST['IA_Telephone'];
	
	
	$Employment_CompanyName 		= $_REQUEST['Employment_CompanyName'];
	$Employment_CompanyAddress 		= $_REQUEST['Employment_CompanyAddress'];
	$Employment_City 				= $_REQUEST['Employment_City'];
	$Employment_Country 			= $_REQUEST['Employment_Country'];
	$Employment_Telephone 			= $_REQUEST['Employment_Telephone'];
	$Employment_WebAddress 			= $_REQUEST['Employment_WebAddress'];
	$IA_ContactPerson 				= $_REQUEST['IA_ContactPerson'];
	//$IA_Name 						= $_REQUEST['IA_Name'];
	//$IA_Address 					= $_REQUEST['IA_Address'];
	//$IA_City 						= $_REQUEST['IA_City'];
	//$IA_Country 					= $_REQUEST['IA_Country'];
	...
	$HltLicense_AuthorityName 		= $_REQUEST['HltLicense_AuthorityName'];
	$HltLicense_AuthorityAddress 	= $_REQUEST['HltLicense_AuthorityAddress'];
	$HltLicense_City 				= $_REQUEST['HltLicense_City'];
	$HltLicense_Country 			= $_REQUEST['HltLicense_Country'];
	$HltLicense_Telephone 			= $_REQUEST['HltLicense_Telephone'];
	$HltLicense_WebAddress 			= $_REQUEST['HltLicense_WebAddress'];
	//$IA_Name 						= $_REQUEST['IA_Name'];
	//$IA_Address 					= $_REQUEST['IA_Address'];
	//$IA_City 						= $_REQUEST['IA_City'];
	//$IA_Country 					= $_REQUEST['IA_Country'];
	...
	function getcheckname($checkID,$savv_check_id){
		...
		if(is_numeric($savv_check_id)){
				...
				$data = getInfo('savvion_check',"id=$savv_check_id");
		...
		$education = '<div  id="eduction">
                                ...
                                <h4 class="section-title">Education</h4>
                                	<h4 class="section-title preface-title">University Details as per Applicant </h4>
                                    <div class="form-group">
                                    	<label for="Education_UniversityName">University Name: </label>
                                    	<textarea name="Education_UniversityName" class="form-control parsley-validated" rows="4" data-parsley-minwords="4" data-parsley-required="true" placeholder="University Name"></textarea>
                                    </div>
                                    <div class="form-group">
                                    	<label for="Education_UniversityAddress">University Address:  </label>
                                    	<input name="Education_UniversityAddress" id="Education_UniversityAddress" value="'.$_REQUEST['Education_UniversityAddress'].'" class="datetimepicker-month form-control" placeholder="University Address">
                                    </div>
                                    <div class="form-group">
                                    	<label for="Education_City">City:  </label>
                                    	<input name="Education_City" id="Education_City" value="'.$_REQUEST['Education_City'].'" class="datetimepicker-month form-control" placeholder="City">
                                    </div>
                                    <div class="form-group">
                                    	<label for="Education_Country">Country:  </label>
                                    	<input name="Education_Country" id="Education_Country" class="datetimepicker-month form-control" placeholder="Country">
                                    </div>
                                    <div class="form-group">
                                    	<label for="Education_Telephone">Telelphone:  </label>
                                    	<input name="Education_Telephone" id="Education_Telephone" class="datetimepicker-month form-control" placeholder="Telelphone">
                                    </div>
									<h4 class="section-title preface-title">University Details as per Masters  </h4>
                                    <div class="form-group">
                                    	<label for="IA_Name">University Name: </label>
                                    	<textarea name="IA_Name" class="form-control parsley-validated" rows="4" data-parsley-minwords="4" data-parsley-required="true" placeholder="University Name"></textarea>
                                    </div>
                                    <div class="form-group">
                                    	<label for="IA_Address">University Address:  </label>
                                    	<input name="IA_Address" id="IA_Address" class="datetimepicker-month form-control" placeholder="University Address">
                                    </div>
                                    <div class="form-group">
                                    	<label for="IA_City">City:  </label>
                                    	<input name="IA_City" id="IA_City" class="datetimepicker-month form-control" placeholder="City">
                                    </div>
                                    <div class="form-group">
                                    	<label for="IA_Country">Country:  </label>
                                    	<input name="IA_Country" id="IA_Country" class="datetimepicker-month form-control" placeholder="Country">
                                    </div>
                                    <div class="form-group">
                                    	<label for="IA_Fax">Fax No:   </label>
                                    	<input name="IA_Fax" id="IA_Fax" class="datetimepicker-month form-control" placeholder="Fax No">
                                    </div>
                                    <div class="form-group">
                                    	<label for="IA_Email">Email ID:  </label>
                                    	<input name="IA_Email" id="IA_Email" class="datetimepicker-month form-control" placeholder="Email ID">
                                    </div>
                                    <div class="form-group">
                                    	<label for="IA_WebAddress">Web Address:  </label>
                                    	<input name="IA_WebAddress" id="IA_WebAddress" class="datetimepicker-month form-control" placeholder="Web Address">
                                    </div>
                                    <div class="form-group">
                                        <label class="checkbox-inline">
                                        <div class="checker"><span><input name="IA_IsFake" type="checkbox" value=""></span></div>
                                        	Is Fake? 
                                        </label>
                                        <label class="checkbox-inline">
                                        <div class="checker"><span><input name="IA_IsOnline" type="checkbox" value=""></span></div>
                                        Is Online ?
                                        </label>
                                    </div>
                                </div>
		...
		$preEmployment  = '<div  id="pre-employment">
                                ...
                                <h4 class="section-title preface-title">Company Details as per Applicant  </h4>
                                    <div class="form-group">
                                        <label for="Employment_CompanyName">Company Name: </label>
                                        <textarea name="Employment_CompanyName" class="form-control parsley-validated" rows="4" data-parsley-minwords="4" data-parsley-required="true" placeholder="Company Name"></textarea>
                                    </div>
                                    <div class="form-group">
                                    	<label for="Employment_CompanyAddress">Company Address:  </label>
                                    	<input name="Employment_CompanyAddress" id="Employment_CompanyAddress" class="datetimepicker-month form-control" placeholder="Company Address">
                                    </div>
                                    <div class="form-group">
                                    	<label for="Employment_City">City:  </label>
                                    	<input name="Employment_City" id="Employment_City" class="datetimepicker-month form-control" placeholder="City">
                                    </div>
                                    <div class="form-group">
                                    	<label for="Employment_Country">Country:  </label>
                                    	<input name="Employment_Country" id="Employment_Country" class="datetimepicker-month form-control" placeholder="Country">
                                    </div>
      								<div class="form-group">
                                    	<label for="Employment_Telephone">Telephone:  </label>
                                    	<input name="Employment_Telephone" id="Employment_Telephone" class="datetimepicker-month form-control" placeholder="Telephone">
                                    </div>
                                    <div class="form-group">
                                    	<label for="Employment_WebAddress">Web Address: </label>
                                    	<input name="Employment_WebAddress" id="Employment_WebAddress" class="datetimepicker-month form-control" placeholder="Web Address">
                                    </div>
                                <h4 class="section-title preface-title">Company Details as per Masters</h4>
                                    <div class="form-group">
                                        <label for="IA_Name">Company Name: </label>
                                        <textarea name="IA_Name" class="form-control parsley-validated" rows="4" data-parsley-minwords="4" data-parsley-required="true" placeholder="Company Name"></textarea>
                                    </div>
                                    <div class="form-group">
                                    	<label for="IA_Address">Company Address: </label>
                                    	<input name="IA_Address" id="IA_Address" class="datetimepicker-month form-control" placeholder="Company Address">
                                    </div>
                                    <div class="form-group">
                                    	<label for="IA_City">City:</label>
                                    	<input name="IA_City" id="IA_City" class="datetimepicker-month form-control" placeholder="City">
                                    ...
                                    <div class="form-group">
                                    	...
                                    	<input name="IA_Telephone" id="IA_Telephone" class="datetimepicker-month form-control" placeholder="Telephone No">
                                    </div>
                                    <div class="form-group">
                                    	<label for="IA_Fax">Fax No: </label>
                                    	<input name="IA_Fax" id="IA_Fax" class="datetimepicker-month form-control" placeholder="Fax No">
                                    ...
                                    <div class="form-group">
                                    	...
                                    	<input name="IA_ContactPerson" id="IA_ContactPerson" class="datetimepicker-month form-control" placeholder="Contact Person">
                                    </div>
		...
		$healthLegislation = '<div id="health-legislation">
                                ...
                                <input name="HltLicense_AuthorityName" id="HltLicense_AuthorityName" class="datetimepicker-month form-control" placeholder="License Authority">
                                </div>
                                <div class="form-group">
                                <label for="HltLicense_AuthorityAddress">Authority Address:  </label>
                                <input name="HltLicense_AuthorityAddress" id="HltLicense_AuthorityAddress" class="datetimepicker-month form-control" placeholder="Authority Address">
                                </div>  
                                <div class="form-group">
                                <label for="HltLicense_City">City:  </label>
                                <input name="HltLicense_City" id="HltLicense_City" class="datetimepicker-month form-control" placeholder="City">
                                </div>
                                <div class="form-group">
                                <label for="HltLicense_Country">Country:  </label>
                                <input name="HltLicense_Country" id="HltLicense_Country" class="datetimepicker-month form-control" placeholder="Country">
                                </div>                               
                                <div class="form-group">
                                <label for="HltLicense_Telephone">Telephone:   </label>
                                <input name="HltLicense_Telephone" id="HltLicense_Telephone" class="datetimepicker-month form-control" placeholder="Telephone">
                                </div>
                                <div class="form-group">
                                <label for="HltLicense_WebAddress">Web Address:  </label>
                                <input name="HltLicense_WebAddress" id="HltLicense_WebAddress" class="datetimepicker-month form-control" placeholder="Web Address">
                                </div>                               
                                <div class="form-group">
                                <label for="IA_Name">Authority Name  </label>
                                <input name="IA_Name" id="IA_Name" class="datetimepicker-month form-control" placeholder="Authority Name">
                                </div>
                                <div class="form-group">
                                <label for="IA_Address">Authority Address  </label>
                                <input name="IA_Address" id="IA_Address" class="datetimepicker-month form-control" placeholder="Authority Address">
                                </div>                               
                                 <div class="form-group">
                                <label for="IA_City">City:  </label>
                                <input name="IA_City" id="IA_City" class="datetimepicker-month form-control" placeholder="City">
