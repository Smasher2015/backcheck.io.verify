			...
			$subject = $subj.' '.$localDate;
				...
				if(!empty($com_users)){
								...
								while($clUser = @mysql_fetch_assoc($com_users)){
									...
									$user_info = getUserInfo($check_added_by);
									$loc_id = $user_info['loc_id'];
									// notification settings // insufficient
									if($clUser['is_insuff_notify']==1 && $a_type=='insufficient'){
									if($loc_id==0){
											// to all users
											//var_dump($com_users); die;
									if($clUser['is_subuser']==0){
									emailTmp($table,$subject,$clUser['email'],"","ops@backcheckgroup.com","","",$fullName);
									}
									}else{
										if($loc_id==$clUser['loc_id'] && $clUser['puser_id']!=0 && $clUser['is_subuser']!=0){
											// to sub user
...
$ch = curl_init();
 ...
 function isLocationWise($com_id){
	...
	function sub_email_notify($posted){
		...
		$res = $db->update("is_other_notify=$is_other_notify,is_weekly_updates=$is_weekly_updates,is_send_searched=$is_send_searched,is_insuff_notify=$is_insuff_notify","users","user_id=$user_id");
		if($res){
