			...
			if($_REQUEST['ERR']==''){
				...
				while(isset($_REQUEST['ename'.$nums])){
					...
					$check_array = array();
							...
							addCICCheckInCase($_REQUEST['checks'.$nums],$VID,$workgroupInfo,$uID);
							}
...
function addMoreAttachments(){
		...
		if($_REQUEST['ERR']==''){
			...
			$nums = 1;
				...
				// insert notificaification to manager about new cases
				$a_info = "More attachments added by ".$_SESSION['fname']." ( $comInfo[name] ) against Candidate <a href=\"".SURL."?action=details&case=".$VID."&_pid=81#check_tab_".$CID."\">".$vInfo[v_name]."</a> ";
				...
				$notify = createNotifications(4,$a_info,$VID,'',$toEmail,'',$CID,$toWhome);
...
function addUnie(){
	...
	if(!isset($_REQUEST['id'])){
		...
		$values = "'$beneficiary',$_POST[ufee],'$rgn','$cty','$uni','$url','$acd','$inf',$uID,CURRENT_TIMESTAMP,'$vendor',$charges,$ddreq,$nfee,'$uni_req'";	

		$cols="uni_ben,uni_fee,uni_region,uni_city,uni_Name,uni_url,uni_ac_url,uni_var,uni_uadd,uni_addate,uni_vendor,uni_vchar,uni_ddr,uni_nfe,uni_req";
		...
		if($uni!=''){
			...
			$values .="uni_vendor='$vendor',uni_vchar=$charges,uni_ddr=$ddreq,uni_nfe=$nfee,uni_req='$uni_req'";
...
function add_client_checks($clid){
	...
	$fullName = $user_info['first_name']." ".$user_info['last_name'];
			...
			$agreement_confg = $db->update("is_expired='1'","client_agreement_confg","comps_id=$clid");
...
The above link will take you to the page with The background check group agreement view.
		  ...
		  emailTmp($table2,'Agreement Modify From BCG',"atta@xcluesiv.com","","","","","$fullName2","");
		  ...
		  $db->update("is_expired='1'","client_agreement","com_id=$clid");
		...
		if(is_numeric($check)){
			...
			//if(mysql_num_rows($record2)>0){
				//$db->update("is_expired='1'","client_agreement","com_id=$clid AND checks_id=$check");
			// }
	   ...
	   if ($error == UPLOAD_ERR_OK){
		...
		// for add
			...
			if(mysql_num_rows($company)==0){
				...
				$cols   = "name,email,ind_id,type,location,address,pymterm,comment,pname,phone,erpid,paid,credits,mode_of_payment,account_type,is_check_wise_pay,can_download_reports,location_wise,monthly_credits_allowed";
...
Background Check Team</p> 
   ...
   <tr>
  	<td align="center" width="100%" colspan="3" bgcolor="#FFFFFF"><p style="padding:17px 17px; color: #54565c; font-size:13px;"><a href="mailto:support@backcheckgroup.com" style="color: #fff;background: #27abc9;text-decoration: none;padding: 15px 41px;font-size: 13px;text-transform: uppercase; border-radius:68px;">have any questions ?</a></p></td>
  </tr>
    <tr>
...
return $rand_id;
 ...
 // by khl
	...
	function createNotifications($level_id,$a_info="",$v_id='',$notify_to='',$to_email='',$a_type='notification',$as_id='',$toWhome=''){
		...
		if($note){
			$subj = ($a_type!='notification')?"New ".ucwords($a_type)." Notification":"New ".ucwords($a_type);
			//$localDate = date("D, M d, Y H:i:s A");
			$localDate="";
			$subject = $subj.' '.$localDate;
				...
				if(!empty($com_users)){
								...
								while($clUser = @mysql_fetch_assoc($com_users)){
									...
									if($loc_id==0){
										...
										if($loc_id==$clUser['loc_id'] && $clUser['puser_id']!=0 && $clUser['is_subuser']!=0){
											// to sub user
									emailTmp($table,$subject,$clUser['email'],"","khalique@xcluesiv.com","","",$fullName);
				...
				//emailTmp($table,$subject,'mis@backcheckgroup.com',"","","","","Team");
							}
				//}
			return true;
		}else{
	...
	function getlogo()
 {
  global $db,$COMINF,$LEVEL; 
	...
	$cols = "co.name as Client, emp_id AS 'Employee ID',vd.v_name AS 'Candidate Name', ck.checks_title AS 'Check title',
			vc.as_status AS 'Working Status' ,vc.as_vstatus AS 'Verification Status' , DATE(vc.as_addate) AS 'Received On', DATE(vc.as_cldate) AS 'Closed On' ,vc.user_id AS 'Analyst Name', vc.as_addate , vc.as_cldate,  vd.com_id $daysCol ".$ifinvoice_select.",uni.uni_Name AS 'IA'";
...
//var_dump($countInsuff); die;
	...
	$a_info = "$countInsuff $Att marked as insufficient by analyst (".SUDONYMS.") of Check:$_POST[chk_name] (Client: $_POST[com_name] and Candidate: <a href=\"".SURL."?action=details&case=$vData[v_id]&_pid=81#check_tab_$as_id\">$vData[v_name]</a>)";
	 $db->update("as_status='Insufficient'","ver_checks","as_id=$as_id");				
	 $notify = createNotifications(4,$a_info,$v_id,'','','insufficient',$as_id,'client');
	}
...
$ch = curl_init();
 ...
 function isLocationWise($com_id){
	...
	function sub_email_notify($posted){
		...
		$res = $db->update("is_other_notify=$is_other_notify,is_weekly_updates=$is_weekly_updates,is_send_searched=$is_send_searched","users","user_id=$user_id");
		if($res){
	...
	while($resData = @mysql_fetch_assoc($selData)){
	if($resData[])	
	}
	...
	addCICCheckInCase($_REQUEST['checks'.$nums],$VID,$workgroupInfo,$uID);
	function addCICCheckInCase($checks,$VID,$workgroupInfo=array(),$uID){
		global $db;
		$vData  = getVerdata($VID);
		$company_id = $vData['com_id'];
		$country = $vData['v_country'];
		//$data = array();
		$check_array = array();
		foreach($checks as $check){
		$chkk = explode("_",$check);
		...
		if(!empty($data)){
			...
			foreach($data as $check=>$price){
				...
				// add to bitrix	
				$task_array=array();
				$task_array['task_name']='Check For '.$vData[v_name] ." - $bCode";
				$task_array['task_desc']="Gender : $vData[gender]
				Father Name : $vData[v_ftname]
				NIC : $vData[v_nic]
				Date of Birth : $vData[v_dob]
