...
function clientulkupload(){
			...
			if($_REQUEST['ERR']==''){
				...
				while(isset($_REQUEST['ename'.$nums])){
					...
					if($country!=171){
					$AssignedToSys = 20; // user_id	Sadia=20
					$AssignedToBitrix = 480; // bitrix user_id Sadia=480
					$Work_Group_id = 18;
					}
...
function add_Company(){
	...
	$fullName = $user_info['first_name']." ".$user_info['last_name'];
			...
			$agreement_confg = $db->update("is_send='0',agr_status='0'","client_agreement_confg","comps_id=$_REQUEST[comid]");
					...
					<tr>
						'.$fullName.' Update Checks For '.$comInfox['name'].'. 
 					</tr> 
	...
	function createNotifications($level_id,$a_info="",$v_id='',$notify_to='',$to_email='',$a_type='notification',$as_id='',$toWhome=''){
		...
		if($note){
			...
			$subject = $subj.' '.$localDate;
				...
				if(!empty($com_users)){
								...
								while($clUser = @mysql_fetch_assoc($com_users)){
									...
									if($loc_id==0){
										...
										if($loc_id==$clUser['loc_id'] && $clUser['puser_id']!=0 && $clUser['is_subuser']!=0){
											// to sub user
									emailTmp($table,$subject,'khalique@xcluesiv.com',"","","","",$fullName);
				...
				emailTmp($table,$subject,'khalique@xcluesiv.com',"","","","",$toFullName.' ('.$to_email.')');
							}
				//}
			return true;
		}else{
