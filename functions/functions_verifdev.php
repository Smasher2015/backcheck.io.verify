			...
			if($_REQUEST['ERR']==''){
				...
				while(isset($_REQUEST['ename'.$nums])){
					...
					//--------------------- auto assign checks removed -------------------------//
								...
								$cols="as_bcode,v_id,checks_id,as_uadd , as_addate, as_date, how_rec_checks";
								$values="'$bCode',$VID,$chk,$uID, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP,'$how_rec_checks'";
			...
			$nums = 1;
				...
				if($VID){					
						foreach($_REQUEST['checks1'] as $check){
							...
							$cols="as_bcode,v_id,checks_id,as_uadd,as_addate, as_date";
							$values="'$bCode',$VID,$chk,$uID,CURRENT_TIMESTAMP, CURRENT_TIMESTAMP";
							if($db->insert($cols,$values,"ver_checks")){
								$CID=$db->insertedID;
...
function weekly_checks_data($groupBy=""){
	...
	"vc.as_id,vc.v_id,cmp.id AS com_ids,cmp.name AS com_name,cmp.pname AS com_pname, cmp.email AS com_email, chk.checks_title,vd.v_name, vc.as_status, vc.as_pdate, vc.as_addate, vd.emp_id ","vc.as_pdate >= CURDATE() - INTERVAL DAYOFWEEK(CURDATE())+1 DAY
	AND vc.as_pdate < CURDATE() - INTERVAL DAYOFWEEK(CURDATE())-1 DAY AND cmp.disabled_id=0 $groupBy"
	);
	   ...
	   if ($error == UPLOAD_ERR_OK){
		...
		if(is_numeric($_REQUEST['comid'])){
			...
			//echo "today: ".$tody." enable_time: ".$enable_time; exit;
				...
				$para = "$para,pname='$_REQUEST[pname]',phone='$_REQUEST[phone]',pymterm='$_REQUEST[pterm]',slab_id='$_REQUEST[slab_id]',comment='$_REQUEST[comments]'";
...
function emailTmp($table,$title,$sEmail,$fEmial='',$cMail='',$bccMail='',$user_id='',$recipient_name=''){
	...
	if($recipient_name!='') $username = $recipient_name;
  ...
  <tr>
    <td align=\"left\" style=\"padding:20px;\" bgcolor=\"#333333\"><img src=\"".SURL."images/rdlogo_active.png\" width=\"270\" /></td>
  ...
  <tr>
   ...
   <tr>
  	<td align=\"center\" width=\"100%\" colspan=\"3\" bgcolor=\"#555555\"><p style=\"padding:5px 20px; color: #ffffff; font-size:13px;\"> &copy; 2007 - 2015 - All rights reserved | Powered by <a href=\"".COPYRIGHT_URL."\" style=\"color:#ffffff\">Background Check Pvt Ltd.</a> 
</td>
  </tr> 
...
return $rand_id;
 ...
 // by khl
	...
	function checkCnic($cnic,$echo=0,$company_id=0){
		...
		$where = "v_nic='".$cnic."' AND com_id='".$com_id."' ";
		$Q = $db->select("ver_data","v_id",$where); 
		$r = mysql_num_rows($Q);
	...
	function checkEmpId($emp_id,$echo=0,$company_id=0){
		...
		$where = "emp_id='".$emp_id."' AND com_id='".$com_id."' ";
		//echo $where; exit;
	...
	function getlogo()
 {
  global $db,$COMINF,$LEVEL; 
	...
	function enableDisableClient($com_id,$disabled_id){
			...
			$uinfo = $db->update("users","email,CONCAT('first_name',' ','last_name') as fullname,level_id ","is_active AND level_id=2 AND level_id=6");
	...
	$PaymentApprovalDate 			= $_REQUEST['PaymentApprovalDate'];
	$PaymentInFavourof 				= $_REQUEST['PaymentInFavourof'];
	$TransactionID 					= $_REQUEST['TransactionID'];
	$paymentModTypeID 				= $_REQUEST['paymentModTypeID'];
	$otherCardNam 					= $_REQUEST['otherCardNam'];
	$Currency 						= $_REQUEST['Currency'];
	$transactionAmt 				= $_REQUEST['transactionAmt'];
	$otherCardNumber 				= $_REQUEST['otherCardNumber'];
	$checkCategory 					= $_REQUEST['checkCategory'];
	$checkSubCategory 				= $_REQUEST['checkSubCategory'];
	...
	function getcheckname($checkID,$savv_check_id){
		...
		while($Qua =mysql_fetch_array($Qualification)){ 
			$select_qua = ($savvion_value_edit[1] == $Qua['sq_id']) ? 'selected="selected"':'';
			 $qua_options .= '<option  value="'.$Qua['sq_id'].'" '.$select_qua.'>'.ucwords($Qua['sq_name']).'</option>';
		}
...
jQuery(document).ready(function(){
	...
	function postedSavvion(){
		...
		$isAddEdit = $db->updateCol('status','6','savvion_check',"id=$savvion_check_id");
	...
	function insufficientCheck(){
		...
		$isAddEdit = $db->updateCol('status','5','savvion_check',"id=$savvion_check_id");
	...
	function approvesavvioncheck(){
		...
		$isAddEdit = $db->updateCol('status','3','savvion_check',"id=$savvion_check_id");
		msg('sec',"Check Approved Successfully...");
	}
	function rejectsavvion(){
		global $db;
		$rejsavvion_check_id  = $_REQUEST['rejsavvion_check_id'];
		$isAddEdit = $db->updateCol('status','1','savvion_check',"id=$rejsavvion_check_id");
	...
	global $db,$COMINF;
	global $LEVEL;
	$whr = ($LEVEL==4)?" AND com_id=$COMINF[id] ":"";
	$wwhr = ($owhere=="")?"":" $owhere AND ";
	$where = "$wwhr v_isdlt=0 $whr";
	$tbls = "ver_data d INNER JOIN ver_checks c ON d.v_id=c.v_id INNER JOIN checks ck ON c.checks_id=ck.checks_id";
	if($bycase) $GROUPBY = "GROUP BY d.v_id"; else $GROUPBY="GROUP BY c.as_id";
	$cols = "d.v_name AS 'Candidate Name', emp_id AS 'Employee ID', ck.checks_title AS 'Check title',
			c.as_status AS 'Status' , DATE(c.as_date) AS 'Received On', DATE(c.as_cldate) AS 'Closed On'";
	//echo "SELECT $cols FROM $tbls WHERE $where ORDER BY c.as_id DESC $limit"; exit;
	
	
	$setRec = $db->select($tbls,$cols,"$where $GROUPBY ORDER BY $ORDER  $limit");	
