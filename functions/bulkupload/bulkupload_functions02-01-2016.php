...
function clientBulkUploadAjax2(){
			...
			if(count($_REQUEST['checks'])==0) msg('err',"You don't have check(s) in your package, please contact system admin!");
			if($_REQUEST['ERR']==''){
				...
				while(isset($_REQUEST['first_name'.$nums])){
					...
					$cols="thum,image,v_first_name,v_last_name,v_name,v_ftname,v_nic,v_dob,com_id,v_recdate,v_bcode,v_uadd, emp_id";
					$values="'$thum','$image','$first_name','$last_name','$ename','$fname','$cnic','$dob',$company_id,'$recdate','$bCode',$uID, '".$empcode."'";
					...
					// khl //	
							...
							//echo "INsert into ver_checks set ($cols) VALUES($values)"; exit;
								if($db->insert($cols,$values,"ver_checks")){
								$CID=$db->insertedID;
					
					// IF EDUCATION CHECK AVAILABLE checks_id=1
					if(in_array($eduCheckID,$_REQUEST['checks'])){
					if($check==$eduCheckID){
					$colsAddata = " (as_id,d_type,d_mtitle,d_stitle) ";
					$valsAddata = " ($CID,'multy','University Name','$uni_name'),($CID,'multy','Registration Number','$reg_num'),($CID,'multy','Degree Title','$degree'),($CID,'multy','Remarks','$remarks'),($CID,'multy','Passing Year','$pass_year'),($CID,'multy','Serial No','$serial_no')";
					//insertSet
					...
					if(in_array($empCheckID,$_REQUEST['checks'])){
					if($check==$empCheckID){
					$colsAddata = " (as_id,d_type,d_mtitle,d_stitle) ";
					$valsAddata = " ($CID,'multy','Company Name','$company_name'),($CID,'multy','Date of Joining','$date_of_join'),($CID,'multy','Employement Status','$emp_status'),($CID,'multy','Last Working Day','$last_work_day'),($CID,'multy','Last Designation','$last_designation'),($CID,'multy','Last Place of Posting','$last_place_posted')";	
					...
					if($isRecords==0) echo "No records added!"; exit;	
