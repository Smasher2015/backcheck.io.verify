...
function getworkgroup(){
 ...
 $query_string="action=getgroups";
    curl_setopt($ch, CURLOPT_URL, "http://vendoronboard.net/rest-api.php");
    // Set a referer
...
function add_Company(){
		...
		if(trim($_REQUEST['pname'])=='') msg('err',"Please input Point of Contact Name!");
		if(trim($_REQUEST['cEmail'])=='') msg('err',"Please input Point of Contact Email!");
		if(trim($_REQUEST['phone'])=='') msg('err',"Please input Point of Contact Phone!");
	...
	if(is_numeric($_REQUEST['group_id'])){	
		...
		if(isset($_REQUEST['check'])){
			...
			if(is_numeric($_REQUEST['check'])){
				...
				$para = "checks_title='$_REQUEST[cktl]',checks_desc='$_REQUEST[ckds]',checks_amt=$_REQUEST[amnt],checks_wdays='$_REQUEST[wdays]',group_id='$_REQUEST[group_id]'";
			...
			if(mysql_num_rows($checks)==0){
				...
				$values = "'$_REQUEST[cktl]','$_REQUEST[ckds]',$_SESSION[user_id],$_REQUEST[amnt],'$_REQUEST[wdays]','$_REQUEST[group_id]'";

				$isIncUp = $db->insert("checks_title,checks_desc,checks_owner,checks_amt,checks_wdays,group_id",$values,"checks");
...
return $rand_id;
	...
	function createNotifications($level_id,$a_info="",$v_id='',$notify_to=''){
		...
		if($note){
			...
			$subject = "New Notification";
				...
				$to_email = 'khalique@xcluesiv.com'; // Manager only;
				
				// for now only to manager 11-06-2015
				if($LEVEL==4){ 
				emailTmp($table,$subject,$to_email,"","","","","Team");
				}
			return true;
		}else{
	...
	function getlogo()
 {
  global $db,$COMINF,$LEVEL; 
	...
	$tbls = "ver_data d INNER JOIN ver_checks c ON d.v_id=c.v_id INNER JOIN checks ck ON c.checks_id=ck.checks_id";
	if($bycase) $GROUPBY = "GROUP BY d.v_id"; else $GROUPBY="GROUP BY c.as_id";
	$cols = "emp_id AS 'Employee ID',d.v_name AS 'Candidate Name', ck.checks_title AS 'Check title',
			c.as_status AS 'Working Status' ,c.as_vstatus AS 'Verification Status' , DATE(c.as_date) AS 'Received On', DATE(c.as_cldate) AS 'Closed On'";
	...
	// by khl
	function ChecksAmount($data,$com_id){
	global $db,$COMINF,$LEVEL;
	$nums=1;
	$totalAmount=0;
	...
	function addClientInvoice($com_id,$cost=0){
	...
	$cols = "com_id,cost,due_date";
	$values = "$com_id,$cost,'$dueDate'";
	...
	// RD/L/99/15-16
	$in_id = $db->insertedID;
	//$invoice_number = "RD/L/".date("y")."/".date("m")."/".$in_id;
	
	//$db->updateCol("invoice_number","'$invoice_number'",'client_invoices',"id=$in_id");
