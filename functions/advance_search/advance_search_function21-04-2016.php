...
function advance_search_case_info($twhere='',$torder='',$isSearch=''){
	...
	//$columns = array("","co.name","v_name","v_ftname","v_date","v_status");
	$columns2 = array("vd.emp_id","v_name","cc.checks_title","","u.first_name","ndate","as_cldate","as_vstatus");
	$columns = array_merge($columns1,$columns2);
	//var_dump($columns);
...
// end ata work
	...
	if( !empty($requestData['search']['value']) ) {
		...
		$totalFiltered = @mysql_num_rows($db->select($tbls,"vd.v_id,DATE(vc.as_cldate), vc.as_addate $daysCol","$where GROUP BY  vc.as_id  $having "));
		...
		$total = @mysql_num_rows($db->select($tbls,"vd.v_id,DATE(vc.as_cldate), vc.as_addate $daysCol","$where GROUP BY  vc.as_id  $having "));
		...
		while($re = mysql_fetch_assoc($cases)) { 
						...
						$as_vstatuss = ($re['as_vstatus'] == "Not Initiated")?$re['as_vstatus']:$re['as_vstatus'].$as_statuss;
