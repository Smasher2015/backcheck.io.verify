...
$table = '<table  border="1" style="border-collapse: collapse; border:1px solid #cccccc; border-color: #cccccc;">';
			...
			$cols = "DATE(as_cldate), vc.user_id,vc.bitrixtid,vd.com_id,as_id,vc.v_id,vc.as_addate as add_date, as_status,checks_title,vd.v_name AS 'Applicant',vd.emp_id AS 'EMP ID', c.name AS 'Client', CONCAT(u.first_name,' ',u.last_name) 
        ...
        while($analyst_arr = mysql_fetch_array($analyst)){
			...
			if($days>$keys && $days<=$vals){
				
				$users = $db->select('users','user_id,concat(first_name,\' \',last_name) as fullname',"user_id='$analyst_arr[user_id]'");
			
			while($rsUsers = @mysql_fetch_assoc($users)){
			$user_id =	$rsUsers['user_id'];
			$fullname =	$rsUsers['fullname'];
			
			$usersCount[$vals]['Not Initiated'][$fullname]	+= countChecks("vc.user_id='$rsUsers[user_id]' AND vc.v_id='$analyst_arr[v_id]'  AND  as_status = 'Open' AND as_vstatus='Not Initiated' AND as_isdlt=0");
			$usersCount[$vals]['Insufficient'][$fullname]	+= countChecks("vc.user_id=$rsUsers[user_id] AND vc.v_id='$analyst_arr[v_id]'  AND  as_status = 'Insufficient' AND as_vstatus='Not Initiated' AND as_isdlt=0");
			$usersCount[$vals]['WIP'][$fullname]	+= countChecks("vc.user_id='$rsUsers[user_id]' AND vc.v_id='$analyst_arr[v_id]' AND as_status = 'Open' AND as_vstatus!='Not Initiated' AND as_isdlt=0 AND as_cldate IS NULL ");
			$usersCount[$vals]['Closed'][$fullname]	+= countChecks("vc.user_id='$rsUsers[user_id]' AND vc.v_id='$analyst_arr[v_id]' AND as_status = 'Close' AND as_qastatus!='Rejected' AND as_isdlt=0 ");
...
$not_initiated1 = $not_initiated1.'</table></td><td>&nbsp;</td>';

}


$not_initiated="<table cellpadding='5'><tr><th align='center' colspan='10'><h2>From $from1 to $to1 </h2></th></tr><tr>$not_initiated1";
...
foreach($value as $fullname => $val){
$not_initiated .= '<tr><td>'.$val.'</td></tr>';
$totalarr[$fullname] += $val;
}
$not_initiated .='</table></td><td>&nbsp;</td>';	
}
$totaltable = "<td>&nbsp;</td><td><table><tr><h3>Total</h3></tr>";
...
$not_initiated="<table cellpadding='5'><tr><th align='center'  colspan='10'><h2>From $from2 to $to2 </h2></th></tr><tr>$not_initiated1";
...
$not_initiated="<table cellpadding='5'><tr><th align='center' colspan='10'><h2>From $from3 to $to3 </h2></th></tr><tr>$not_initiated1";
...
$not_initiated="<table cellpadding='5'><tr><th align='center' colspan='10'><h2>From $from4 to $to4 </h2></th></tr><tr>$not_initiated1";
...
$not_initiated="<table cellpadding='5'><tr><th align='center' colspan='10'><h2>Greater Than 60 </h2></th></tr><tr>$not_initiated1";
...
echo $emailData.'</td></tr></table>';
$subject = "Daily update of all analyst $localDate";
//emailTmp($emailData,$subject,$to_email,'','','','',"Khalique");
